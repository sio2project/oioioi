...
RUN chown -R oioioi:oioioi /sio2/logs
...
RUN echo "[{rabbit, [{tcp_listeners, [5672]}, {loopback_users, []}]}]." | tee /etc/rabbitmq/rabbitmq.config
RUN echo "\"SERVER_ERL_ARGS="+K true +A 4 +P 1048576 -kernel\"          | tee /etc/rabbitmq/rabbitmq-env.conf
...
RUN sed -i -e "s/django.db.backends./django.db.backends.postgresql_psycopg2/g;\
           s/'NAME': ''/'NAME': 'oioioi'/g;\
           s/'USER': ''/'USER': 'oioioi'/g;\
           s/'HOST': '',/'HOST': 'db',/g;\
           s/'PASSWORD': ''/'PASSWORD': 'oioioi'/g;\
...
RUN sed -i -e "s|twistd|/home/oioioi/.local/bin/twistd|g"\
		   -e "s|{{ PROJECT_DIR }}/logs|/sio2/logs|g"\
		   -e "s|filetracker-server|/home/oioioi/.local/bin/filetracker-server|g"\
			   supervisord.conf
