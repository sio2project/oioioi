...
def adjust_unicode_field(
    ...
    form.fields[field_name].validators += [UnicodeValidator(unicode_categories=unicode_categories, allow_spaces=allow_spaces)]
...
def adjust_preferences_factory_fields():
    ...
    if settings.USE_ACE_EDITOR:
        PreferencesFactory.add_field(
            "enable_editor", BooleanField, lambda name, user: handle_enable_editor(user), label=_("Enable editor"), order=0, required=False
        )
...
class RegistrationFormWithNames(RegistrationForm):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.fields.update({"captcha": CaptchaField(label="", widget=CustomCaptchaTextInput)})
...
class OioioiPasswordResetForm(PasswordResetForm):
    error_messages = {
        "unknown": _("That email address doesn't have an associated local user account."),
    }
