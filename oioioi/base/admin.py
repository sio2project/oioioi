...
@admin.action(description=_("Delete selected %(verbose_name_plural)s"))
def delete_selected(modeladmin, request, queryset, **kwargs):
    ...
    to_delete, perms_needed, protected = collect_deleted_objects(modeladmin, request, queryset)
...
class OioioiUserAdmin(UserAdmin, ObjectWithMixins, metaclass=ModelAdminMeta):
    ...
    @admin.action(description=_("Mark users as active"))
...
class MixinsAdmin(InstanceDependentAdmin):
    def __init__(self, *args, **kwargs):
        ...
        if not issubclass(self.default_model_admin, ObjectWithMixins):
            raise AssertionError("MixinsAdmin.default_model_admin must be a subclass of ObjectWithMixins")
