...
class RegistrationView(DefaultRegistrationView):
    def form_class(self, instance=None, *args, **kwargs):
        return PreferencesFactory().create_form(RegistrationFormWithNames, instance, *args, **kwargs)
...
if not settings.SEND_USER_ACTIVATION_EMAIL:
    urlpatterns += [
        path(
            "sign-up/",
            RegistrationView.as_view(success_url=reverse_lazy("sign-up_complete")),
            name="sign-up",
        ),
        path(
            "sign-up/complete/",
            TemplateView.as_view(template_name="registration/registration_and_activation_complete.html"),
            name="sign-up_complete",
        ),
    ]
else:
    urlpatterns += [path("sign-up/", RegistrationView.as_view(), name="sign-up")]
