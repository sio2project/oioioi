...
def _get_user_q_expression(substr, user_field_name=None):
    ...
    if len(substr) > 2:
        ...
        q_dict_two_first = {_prefix(user_field_name, "first_name__icontains"): " ".join(substr)}
...
@jsonify
def get_user_hints_view(request, request_field_name, queryset=None, user_field_name=None):
    user_hints = _get_user_hints(request.GET.get(request_field_name, ""), queryset, user_field_name)
...
def _parse_user_hint(value, queryset=None, user_field_name=None):
    ...
    if len(value) == 1 or (len(value) > 1 and value[1].startswith("(") and value[-1].endswith(")")):
        value = value[0]
    ...
    def to_python(self, value):
        ...
        if isinstance(value, User):
            ...
            if value and not user:
                raise ValidationError(_("User not found or you do not have access to this user account: %s") % (value,))
        return user
