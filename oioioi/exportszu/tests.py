...
class TestSubmissionsWithUserDataCollector(TestCase):
    ...
    def assert_correct_submission_data(self, submission_data_list):
        for s in submission_data_list:
            ...
            self.assertEqual(s.problem_short_name, submission.problem_instance.short_name)
            ...
            self.assertEqual(s.solution_language, os.path.splitext(s.source_file.name)[1][1:])
...
INDEX_HEADER = "submission_id,user_id,username,first_name,last_name,city,school,school_city,problem_short_name,score\r\n"
...
class TestExportSubmissionsView(TestCase):
    ...
    def test_download(self):
        ...
        response = self.client.post("/c/c/export_submissions/", {"round": "", "only_final": "on"})
        self.assertEqual(response.status_code, 200)
        self.assertEqual(response["Content-Disposition"], 'attachment; filename="c.tgz"')
