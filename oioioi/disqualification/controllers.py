...
class DisqualificationContestControllerMixin(object):
    ...
    def is_any_submission_to_problem_disqualified(self, user, problem_instance):
        ...
        """
        return Disqualification.objects.filter(
            user=user, submission__problem_instance=problem_instance, guilty=True
        ).exists()
    ...
    def is_user_disqualified(self, request, user):
        ...
        """
        return Disqualification.objects.filter(
            contest=request.contest, user=user, guilty=True
        ).exists()
    ...
    def change_submission_kind(self, submission, kind):
        ...
        super(DisqualificationContestControllerMixin, self).change_submission_kind(
            submission, kind
        )
    ...
    def _render_contestwide_disqualification_reason(self, request, user):
        ...
        This method is only used internally.
        """
        reasons = Disqualification.objects.filter(
            user=user, contest=request.contest, submission__isnull=True
        )
        if not is_contest_admin(request):
            reasons = reasons.filter(guilty=True)
    ...
    def render_my_submissions_header(self, request, submissions):
        header = super(
            DisqualificationContestControllerMixin, self
    ...
    def render_disqualifications(self, request, user, submissions):
        ...
        """
        if not (
            self.is_user_disqualified(request, user)
            or (
...
class DisqualificationProgrammingContestControllerMixin(object):
    ...
    def render_submission(self, request, submission):
        prev = super(
            DisqualificationProgrammingContestControllerMixin, self
        ).render_submission(request, submission)

        if self.is_submission_disqualified(submission) or (
            is_contest_admin(request) and self.has_disqualification_history(submission)
        ):
...
class WithDisqualificationRankingControllerMixin(object):
    ...
    def filter_users_for_ranking(self, key, queryset):
        qs = super(
            WithDisqualificationRankingControllerMixin, self
    ...
    def _render_ranking_page(self, key, data, page):
        ...
        return render_to_string(
            'disqualification/default-ranking.html', context=data, request=request
        )
    ...
    def _get_csv_row(self, key, row):
        line = super(WithDisqualificationRankingControllerMixin, self)._get_csv_row(
            key, row
        )
        if self._show_disqualified(key):
    ...
    def _annotate_disqualified(self, key, data):
        ...
        not_disqualified = self.contest.controller.exclude_disqualified_users(
