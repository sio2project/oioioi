...
class AcceptMailSubmissionForm(forms.Form):
    ...
    submission_hash = forms.RegexField(
        ...
        error_messages={"invalid": _("Value must consist of numbers or letters from A to F")},
    )
    ...
    def clean(self):
        ...
        try:
            ...
            raise ValidationError(_("Postal submission number %s does not exist") % (mailsubmission_id,))

        if mailsubmission.problem_instance.contest != self.request.contest:
            raise ValidationError(_("Postal submission number %s is for a different contest") % (mailsubmission_id,))
