...
def make_problemtree(tree, root, stdout):
    for family in tree:
        ...
        for edition in family['editions']:
            ...
            try:
                edition_tag = Tag.objects.get(name=edition['tag']).problems.all()
            except ObjectDoesNotExist:
                stdout.write('Ignoring edition with tag {}'.format(edition['tag']))
            ...
            for header, tasks in edition['tasks']:
                ...
                for task in tasks:
                    try:
                        taskname_tag = Tag.objects.get(name=task).problems.all()

                        try:
                            task_obj = (edition_tag & taskname_tag)[0]
                        except IndexError:
                            stdout.write(
                                'Ignoring task: {} from {}, no such task in DB'.format(
                                    task, edition['tag']
                                )
                            )
                            continue
                        ...
                        l.append(
                            reverse(
                                'problem_site',
                                kwargs={'site_key': task_obj.problemsite.url_key},
                            )
                        )
                    except ObjectDoesNotExist:
                        stdout.write(
                            'Ignoring task: {} from {}, task\'s \
                        tag does not exist'.format(
                                task, edition['tag']
                            )
                        )
    ...
    root_content = NodeLanguageVersion(node=root, language=settings.LANGUAGE_CODE)
...
class Command(BaseCommand):
    help = _(
        "Creates portal from json file with structure same like "
