...
def main_page_view(request):
    return redirect(reverse("global_portal", kwargs={"link_name": "default", "portal_path": ""}))
...
def redirect_old_global_portal(request, portal_path):
    ...
    return redirect(reverse("global_portal", kwargs={"link_name": "default", "portal_path": portal_path}))
...
def create_root_node(lang):
    ...
    NodeLanguageVersion.objects.create(language=lang, full_name=name, panel_code=body, node=root)
    return root
...
def create_global_portal_view(request):
    if request.method == "POST":
        ...
        if "confirmation" in request.POST:
            ...
            return redirect(reverse("portals_main_page_type", kwargs={"view_type": "global"}))
    else:
        form = LinkNameForm()
...
def show_node_view(request):
    rendered_panel = mark_safe(render_panel(request, request.current_node.get_lang_version(request).panel_code))
    return render(request, "portals/show-node.html", {"rendered_panel": rendered_panel})


@register_node_action("edit_node", condition=is_portal_admin, menu_text=_("Edit page"), menu_order=200)
...
@register_node_action("add_node", condition=is_portal_admin, menu_text=_("Add a subpage"), menu_order=300)
def add_node_view(request):
    if request.method != "POST":
        form = NodeForm(initial={"parent": request.current_node})
        current_language = get_language_from_request(request)
        formset = NodeLanguageVersionFormset(
            initial=[{"language": lang_short, "DELETE": lang_short != current_language} for lang_short, lang_name in settings.LANGUAGES],
...
def portal_tree_json_view(request):
    ...
    json_tree = render_to_string("portals/portal-tree.json", {"nodes": nodes}, request=request)
...
def portals_main_page_view(request, view_type="public"):
    if request.user.is_superuser:
        ...
        if request.method == "GET":
            ...
            portal_search_q_expr = Q(owner__username__icontains=query) | Q(root__language_versions__full_name__icontains=query) | Q(link_name__icontains=query)
            if view_type == "public":
...
def problem_site_related_portal_pages(request, problem):
    ...
    pages = [(page.get_lang_version(request), page.get_root().portal) for page in pages if page.get_root().portal.is_public]
