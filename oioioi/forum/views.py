...
def forum_view(request):
    category_set = (
        request.contest.forum.category_set.annotate(
            thread_count=Count("thread", distinct=True),
            post_count=Count("thread__post", distinct=True),
            reported_count=Count("thread__post", filter=Q(thread__post__reported=True), distinct=True),
        )
        .order_by("order")
        .all()
    )
...
def category_view(request, category_id):
    ...
    threads = category.thread_set.prefetch_related("post_set").select_related("last_post", "last_post__author").all()
...
def delete_post_view(request, category_id, thread_id, post_id):
    ...
    if not (  # we assert following:
        ...
        choice = confirmation_view(request, "forum/confirm_delete_post.html", {"thread": thread, "post": post})
...
@enforce_condition(not_anonymous & contest_exists & can_enter_contest)
@enforce_condition(forum_exists_and_visible & is_proper_forum & can_interact_with_admins)
...
def post_toggle_reaction(request, category_id, thread_id, post_id):
    ...
    if reaction.exists():
        ...
        PostReaction.objects.create(author=request.user, post_id=post.id, type_of_reaction=reaction_type)
...
def delete_thread_view(request, category_id, thread_id):
    ...
    if choice:
        thread.delete()
    return redirect("forum_category", contest_id=request.contest.id, category_id=category.id)
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            if form.cleaned_data["delete_reports"]:
                removed_reports_count = Post.objects.filter(reported=True, reported_by=user, thread__category__forum=forum).update(
                    reported=False, reported_by=None
                )
                messages.success(request, _("Removed %d reports") % removed_reports_count)
            return redirect(redirect_url)
    else:
        form = BanForm()
