...
from oioioi.contests.utils import is_contest_admin, is_contest_archived
...
def make_list_elem(elem, text=None):
    if not text:
        text = elem.name
    return f"<li>{make_html_link(elem.get_admin_url(), text)}</li>"
...
class ForumAdmin(admin.ModelAdmin):
    ...
    def categories(self, obj):
        slist = [make_list_elem(c) for c in obj.category_set.prefetch_related("thread_set", "thread_set__post_set").all()]
        ret = "".join(slist)
        if not ret:
            ret = string_concat("<li>", _("Empty forum"), "</li>")
        return mark_safe(f"<ul>{ret}</ul>")
...
class CategoryAdmin(admin.ModelAdmin):
    ...
    def threads(self, obj):
        slist = [make_list_elem(t) for t in obj.thread_set.prefetch_related("post_set").all()]
        ret = "".join(slist)
        if not ret:
...
class PostAdmin(admin.ModelAdmin):
    ...
    def approve_action(self, request, queryset):
        ...
        self.message_user(
            request,
            ngettext_lazy("One post was approved.", "%(counter)d posts were approved.", counter) % {"counter": counter},
        )
