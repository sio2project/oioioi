...
class TestForum(TestCase):
    ...
    def test_no_forum_menu(self):
        ...
        Participant.objects.create(contest=contest, user=User.objects.get(username="test_user"))
...
class TestCategory(TestCase):
    ...
    def test_new_categories_are_in_order(self):
        ...
        self.assertEqual(
            list(Category.objects.filter(forum=forum).filter(id__in=categories_ids).order_by("order").values_list("id", flat=True)),
            categories_ids,
        )
    ...
    def test_category_move_up_down(self):
        ...
        [a, b, c] = [Category.objects.create(forum=forum, name=str(i)) for i in range(3)]
        self.assertTrue(self.client.login(username="test_admin"))
    ...
    def test_toggling_reactions(self):
        ...
        toggle_reactions_url = reverse("forum_category_toggle_reactions", kwargs={"category_id": self.category.id})
...
class TestThread(TestCase):
    ...
    def test_remove_posts(self):
        p0 = Post(thread=self.thr, content="test0", author=self.user, add_date=self.past)
        p0.save()
...
class TestLatestPosts(TestCase):
    ...
    def test_paging(self):
        ...
        response = self.client.get(self.url, follow=True)
        self.assertEqual(200, response.status_code)
        self.assertContains(response, f"Thread {p.thread.name} #{p.id}")
        ...
        response = self.client.get(self.url + f"?page={last_page}", follow=True)
        self.assertEqual(200, response.status_code)
        self.assertEqual(
            len(re.findall(r"Thread .*? #\d+", response.content.decode("utf-8"))),
            posts_on_last_page,
    ...
    def test_reaction_display(self):
        ...
        PostReaction(post_id=p.id, type_of_reaction="UPVOTE", author=self.user).save()
...
class TestPost(TestCase):
    ...
    def setUp(self):
        ...
        self.thr = Thread(category=self.cat, name="test_thread")
        self.thr.save()
        self.p = Post(thread=self.thr, content="Test post!", author=self.user, add_date=self.past)
        self.p.save()
    ...
    def test_reactions_from_multiple_users(self):
        ...
        self.assertEqual(2, self.p.reactions.filter(type_of_reaction="DOWNVOTE").count())
...
class TestBan(TestCase):
    ...
    def test_add_thread(self):
        ...
        self.assertEqual(User.objects.get(username="test_user"), thread.last_post.author)
