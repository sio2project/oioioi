...
class Migration(SchemaMigration):
    """Both migrations **will** cause the loss of current records in the
       database. If the queue is active around the time of migration, some of
       the submissions might not be displayed properly in the queue or even
       not displayed there at all.
    """
    ...
    def backwards(self, orm):
        db.delete_table(u'submitsqueue_queuedsubmit')
        db.create_table(u'submitsqueue_queuedsubmit', (
            ('submission', self.gf('django.db.models.fields.related.ForeignKey')(primary_key=True, to=orm['contests.Submission'])),
            ('state', self.gf('oioioi.base.fields.EnumField')(default='QUEUED', max_length=64)),
            ('creation_date', self.gf('django.db.models.fields.DateTimeField')(default=datetime.datetime.now)),
            ('celery_task_id', self.gf('django.db.models.fields.CharField')(max_length=50, unique=True, null=True, blank=True)),
        ))
    ...
    models = {
        ...
        u'submitsqueue.queuedsubmit': {
            'Meta': {'ordering': "['pk']", 'object_name': 'QueuedSubmit'},
