...
@contest_admin_menu_registry.register_decorator(
    _("Statistics"),
    lambda request: reverse("statistics_main", kwargs={"contest_id": request.contest.id}),
...
@enforce_condition(contest_exists & can_enter_contest & can_see_stats & any_statistics_avaiable)
def statistics_view(request, category=statistics_categories["CONTEST"][1], object_name=""):
    controller = request.contest.controller
    ...
    if category == "PROBLEM":
        object = ProblemInstance.objects.get(short_name=object_name, contest=request.contest)
    ...
    data_list = [controller.statistics_data(request, plot_kind, object) for plot_kind, object_name in plots]
...
@contest_admin_menu_registry.register_decorator(
    _("Monitoring"),
    lambda request: reverse("monitoring", kwargs={"contest_id": request.contest.id}),
    condition=(is_contest_admin | is_contest_observer),
    order=110,
)
...
def monitoring_view(request):
    ...
    sys_error_count = (
        SubmissionReport.objects.filter(status="ACTIVE", failurereport__isnull=False, submission__problem_instance__contest=request.contest).count()
        + SubmissionReport.objects.filter(status="ACTIVE", scorereport__status="SE", submission__problem_instance__contest=request.contest).count()
    )

    unanswered_questions = Message.objects.filter(kind="QUESTION", message=None, contest=request.contest).count()
    oldest_unanswered_question = Message.objects.filter(kind="QUESTION", message=None, contest=request.contest).order_by("date").first()
    ...
    round_time_extensions = (
        RoundTimeExtension.objects.filter(round__contest=request.contest.id)
...
def get_rounds_info(request):
    ...
    for round_, rt in rounds_times(request, request.contest).items():
        ...
        if rt.start:
            round_time_info["start_relative"] = naturaldelta(rt.start - request.timestamp) if rt.is_future(request.timestamp) else _("Started")
        else:
            round_time_info["start_relative"] = _("Not set")
        round_time_info["end"] = rt.end or _("Not set")
        if rt.end:
            round_time_info["end_relative"] = naturaldelta(rt.end - request.timestamp) if not rt.is_past(request.timestamp) else _("Finished")
        else:
...
def get_attachments_info(request):
    ...
    for attachment in attachments:
        ...
        if attachment.pub_date:
            pub_date_relative = naturaldelta(attachment.pub_date - request.timestamp) if attachment.pub_date > request.timestamp else _("Published")
        attachment.pub_date_relative = pub_date_relative
    return attachments


def get_permissions_info(request):
    permissions_info = {
        permission_name: (ContestPermission.objects.filter(contest_id=request.contest.id, permission=permission_cls).count())
...
def get_tests_info(request):
    ...
    for t_info in tests_limits:
        ...
        tests_info[t_info["problem_instance__round__name"]][t_info["problem_instance"]]["problem_name"] = t_info["problem_instance__short_name"]
        tests_info[t_info["problem_instance__round__name"]][t_info["problem_instance"]]["submissions_limit"] = t_info["problem_instance__submissions_limit"]
    ...
    solved_qs = ScoreReport.objects.filter(
        submission_report__submission__problem_instance__contest=request.contest,
