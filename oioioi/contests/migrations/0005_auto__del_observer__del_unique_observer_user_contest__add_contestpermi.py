...
class Migration(SchemaMigration):
    ...
    def forwards(self, orm):
        # Adding model 'ContestPermission'
        db.create_table('contests_contestpermission', (
            ('id', self.gf('django.db.models.fields.AutoField')(primary_key=True)),
            ('user', self.gf('django.db.models.fields.related.ForeignKey')(to=orm['auth.User'])),
            ('contest', self.gf('django.db.models.fields.related.ForeignKey')(to=orm['contests.Contest'])),
            ('permission', self.gf('oioioi.base.fields.EnumField')(default='contests.contest_admin', max_length=64)),
        ))
        db.send_create_signal('contests', ['ContestPermission'])

        # Adding unique constraint on 'ContestPermission', fields ['user', 'contest', 'permission']
        db.create_unique('contests_contestpermission', ['user_id', 'contest_id', 'permission'])

        # Removing unique constraint on 'Observer', fields ['user', 'contest']
        db.delete_unique('contests_observer', ['user_id', 'contest_id'])

        # Deleting model 'Observer'
        db.delete_table('contests_observer')


    def backwards(self, orm):
        # Adding model 'Observer'
        db.create_table('contests_observer', (
            ('user', self.gf('django.db.models.fields.related.ForeignKey')(to=orm['auth.User'])),
            ('id', self.gf('django.db.models.fields.AutoField')(primary_key=True)),
            ('contest', self.gf('django.db.models.fields.related.ForeignKey')(to=orm['contests.Contest'])),
        ))
        db.send_create_signal('contests', ['Observer'])

        # Adding unique constraint on 'Observer', fields ['user', 'contest']
        db.create_unique('contests_observer', ['user_id', 'contest_id'])

        # Removing unique constraint on 'ContestPermission', fields ['user', 'contest', 'permission']
        db.delete_unique('contests_contestpermission', ['user_id', 'contest_id', 'permission'])

        # Deleting model 'ContestPermission'
        db.delete_table('contests_contestpermission')


    # XXX: THE METADATA BELOW IS INCORRECT. FORTUNATELY, IT SHOULD NOT BE NEEDED
    #      ANYMORE. NEXT MIGRATION IS OK.
