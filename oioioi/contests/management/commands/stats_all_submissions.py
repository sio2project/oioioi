...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        subs_all = Submission.objects.filter(
            date__gte=datetime.now() - timedelta(days=numdays)
        )
        ...
        for iteration in range(numdays):
            ...
            q = Q(date__gte=datetime.now() - timedelta(days=iteration)) & Q(
                date__lte=datetime.now() - timedelta(days=(iteration - 1))
            )
