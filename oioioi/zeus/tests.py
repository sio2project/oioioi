...
class ZeusJsonTest(TestCase):
    ...
    def test_json_base64_identity(self):
        self.assertEqual(
            self._ob, self._json_base64_decode(_json_base64_encode(self._ob))
        )
        self.assertEqual(
            self._ob_json, _json_base64_encode(self._json_base64_decode(self._ob_json))
        )
        for j in self._jsons:
...
class ZeusViewsTest(TestCase):
    ...
    def test_push_grade(self):
        ...
        response = self.client.post(
            url, _json_base64_encode(data), follow=True, content_type="application/json"
        )
