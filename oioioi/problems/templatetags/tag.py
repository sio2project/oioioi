...
def prefetch_top_tag_proposals(problems):
    ...
    prefetch_related_objects(
        ...
        Prefetch(
            ...
            queryset=AggregatedAlgorithmTagProposal.objects.filter(amount__gte=min_proposals_per_tag).order_by("-amount")[:max_proposals_shown],
    ...
    for problem in problems:
        ...
        problem.top_tag_proposals = [proposal for proposal in problem.top_tag_proposals if proposal.tag.pk not in algo_tag_pks]
...
def tag_label(tag):
    ...
    return format_html(
        '<a title="{tooltip}" class="badge tag-label tag-label-{cls}" href="{href}" >{name}</a>',
