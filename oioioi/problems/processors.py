...
def dangling_problems_processor(request):
    ...
    dangling_pis = ProblemInstance.objects.filter(contest=request.contest, round__isnull=True)
...
def problems_need_rejudge_processor(request):
    ...
    pis = ProblemInstance.objects.filter(contest=request.contest, needs_rejudge=True)
    count = pis.count()
