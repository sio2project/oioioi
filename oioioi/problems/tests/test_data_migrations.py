...
populate_aggregated_tag_proposals = migration_module.populate_aggregated_tag_proposals


def _get_tag_amounts(aggregated_model, problem):
    """Returns a dictionary mapping tags to their amounts for a given problem."""
    return {proposal.tag: proposal.amount for proposal in aggregated_model.objects.filter(problem=problem)}
...
class PopulateAggregatedTagProposalsTest(TestCase):
    ...
    def test_populate_aggregated_tag_proposals(self):
        ...
        self.assertEqual(_get_tag_amounts(AggregatedAlgorithmTagProposal, self.problem1), {self.algorithm_tag1: 2, self.algorithm_tag2: 1})
        self.assertEqual(_get_tag_amounts(AggregatedAlgorithmTagProposal, self.problem2), {self.algorithm_tag1: 1, self.algorithm_tag2: 3})

        self.assertEqual(DifficultyTagProposal.objects.count(), 4)
        self.assertEqual(AggregatedDifficultyTagProposal.objects.count(), 3)
        self.assertEqual(_get_tag_amounts(AggregatedDifficultyTagProposal, self.problem1), {self.difficulty_tag1: 2, self.difficulty_tag2: 1})
        self.assertEqual(_get_tag_amounts(AggregatedDifficultyTagProposal, self.problem2), {self.difficulty_tag2: 1})
