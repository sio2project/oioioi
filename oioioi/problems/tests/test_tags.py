...
class TestAlgorithmTagLabel(TestCase):
    ...
    def get_tag_labels(self, parameters):
        url = f"{reverse(self.view_name)}?{urllib.parse.urlencode(parameters)}"
        return self.client.get(url)

    def test_algorithm_tag_label_view(self):
        response = self.get_tag_labels({"name": "Najdłuższy wspólny podciąg rosnący", "proposed": "-1"})
        self.assertEqual(response.status_code, 200)
        self.assertContains(response, b"Najd\u0142u\u017cszy wsp\u00f3lny podci\u0105g rosn\u0105cy")

        response = self.get_tag_labels({"name": "Programowanie dynamiczne", "proposed": "-1"})
        self.assertEqual(response.status_code, 200)
...
class TestProblemSearchOrigin(TestCase, AssertContainsOnlyMixin):
    ...
    def test_search_origininfovalue_multiple(self):
        ...
        self.assertEqual(response.status_code, 200)
        self.assert_contains_only(response, ["3_pa_2011_r1", "3_pa_2011_r2", "3_pa_2012_r1"])

        response = self.client.get(self.url, {"origin": ["pa_2011", "pa_2012", "pa_r1", "pa_r2"]})
        self.assertEqual(response.status_code, 200)
        self.assert_contains_only(response, ["3_pa_2011_r1", "3_pa_2011_r2", "3_pa_2012_r1"])
...
class TestProblemSearchHintsTags(TestCase, AssertContainsOnlyMixin):
    ...
    def test_category_hints(self):
        self.client.get("/c/c/")
        response = self.client.get(self.category_url, {"category": "round", "q": "Potyczki Algorytmiczne"})
        self.assertEqual(response.status_code, 200)
    ...
    def test_selected_origintag_hints_en(self):
        self.client.get("/c/c/")
        response = self.client.get(self.selected_origintag_url, {"q": "Potyczki Algorytmiczne"})
        self.assertEqual(response.status_code, 200)
