...
class TestVisibilityMigrationReverse(TestCaseMigrations):
    ...
    def test(self):
        ...
        self.assertEqual(Problem.objects.get(id=self.friends_problem_id).is_public, False)
        self.assertEqual(Problem.objects.get(id=self.private_problem_id).is_public, False)
