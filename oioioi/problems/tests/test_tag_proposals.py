...
class TestAlgorithmTagsProposalHintsBase(TestCase):
    ...
    def get_query_url(self, parameters):
        return f"{reverse(self.view_name)}?{urllib.parse.urlencode(parameters)}"
...
class TestAlgorithmTagsProposalHintsEnglish(TestAlgorithmTagsProposalHintsBase):
    def test_tag_proposal_hints_view(self):
        ...
        self.assertNotContains(response, b"Najd\u0142u\u017cszy wsp\u00f3lny podci\u0105g rosn\u0105cy")
...
class TestAlgorithmTagsProposalHintsPolish(TestAlgorithmTagsProposalHintsBase):
    def test_tag_proposal_hints_view(self):
        ...
        self.assertEqual(response.status_code, 200)
        self.assertContains(response, b"Najd\u0142u\u017cszy wsp\u00f3lny podci\u0105g rosn\u0105cy")
        self.assertNotContains(response, "Longest common increasing subsequence")
        self.assertNotContains(response, "lcis")
...
def _get_tag_name_amounts(aggregated_model, problem):
    ...
    return {proposal.tag.name: proposal.amount for proposal in aggregated_model.objects.filter(problem=problem)}
...
class TestSaveProposals(TestCase):
    ...
    def test_save_proposals_view(self):
        ...
        self.assertTrue(AlgorithmTagProposal.objects.filter(problem=problem, tag=AlgorithmTag.objects.get(name="dp"), user=user).exists())
        self.assertTrue(
        ...
        self.assertTrue(DifficultyTagProposal.objects.filter(problem=problem, tag=DifficultyTag.objects.get(name="easy"), user=user).exists())
        self.assertEqual(
        ...
        self.assertTrue(AlgorithmTagProposal.objects.filter(problem=problem, tag=AlgorithmTag.objects.get(name="lcis"), user=user).exists())
        self.assertTrue(AlgorithmTagProposal.objects.filter(problem=problem, tag=AlgorithmTag.objects.get(name="dp"), user=user).exists())
        self.assertTrue(
            AlgorithmTagProposal.objects.filter(
                problem=problem,
        ...
        self.assertTrue(DifficultyTagProposal.objects.filter(problem=problem, tag=DifficultyTag.objects.get(name="medium"), user=user).exists())
        self.assertEqual(
            _get_tag_name_amounts(AggregatedDifficultyTagProposal, problem),
            {"easy": 1, "medium": 1},
        )
