...
class TestTaskArchive(TestCase):
    ...
    def test_unicode_names(self):
        ...
        self.assertEqual(ic.full_name, "Dzień")
        iv = OriginInfoValue.objects.get(pk=4)
        self.assertEqual(iv.full_name, "Olimpiada Informatyczna Finał")
    ...
    def test_task_archive_progress_labels(self):
        ...
        pos = html.find("text-bg-danger")
        self.assertTrue(pos == -1)
        pos = html.find("text-bg-warning")
        self.assertTrue(pos == -1)
        pos = html.find('<a class="badge text-bg-success" href="/s/2/"> 100</a>')
        self.assertTrue(pos != -1)
        ...
        pos = html.find('<a class="badge text-bg-danger" href="/s/3/"> 0</a>')
        self.assertTrue(pos != -1)
        pos = html.find('<a class="badge text-bg-warning" href="/s/6/"> 50</a>')
        self.assertTrue(pos != -1)
        pos = html.find("text-bg-success")
        self.assertTrue(pos == -1)

        def test_can_access_with_result(score, max_score):
            user = User.objects.get(username="test_user2")
            problem_instance = ProblemInstance.objects.get(pk=4)
            submission = Submission.objects.create(problem_instance=problem_instance, score=score, user=user)
            submission_report = SubmissionReport.objects.create(kind="NORMAL", submission=submission)
