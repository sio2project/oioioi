...
@problem_site_tab(_("Problem statement"), key="statement", order=100, condition=check_for_statement)
def problem_site_statement(request, problem):
    statement = query_statement(problem.id)
    if not statement:
        statement_html = render_to_string("problems/no-problem-statement.html", {"problem": problem, "can_admin_problem": can_admin_problem(request, problem)})
        ...
        statement_html = render_to_string(
            ...
            {"problem": problem, "statement_url": statement_url, "can_admin_problem": can_admin_problem(request, problem)},
        )
...
def check_for_downloads(request, problem):
    ...
    return bool(ProblemAttachment.objects.filter(problem=problem)) or bool(attachment_registry_problemset.to_list(request=request, problem=problem))


@problem_site_tab(_("Downloads"), key="files", order=200, condition=check_for_downloads)
def problem_site_files(request, problem):
    additional_files = sorted(attachment_registry_problemset.to_list(request=request, problem=problem), key=itemgetter("name"))
...
def problem_site_submissions(request, problem):
    ...
    if request.user.is_authenticated:
        qs = controller.filter_my_visible_submissions(
            request,
            Submission.objects.filter(problem_instance=problem.main_problem_instance).order_by("-date"),
        )
    else:
        qs = []
...
def problem_site_submit(request, problem):
    ...
    if request.method == "POST":
        form = SubmissionFormForProblemInstance(request, pi, request.POST, request.FILES)
        if form.is_valid():
    ...
    return TemplateResponse(request, "problems/submit.html", {"problem": problem, "form": form})
...
def problem_site_secret_key(request, problem):
    return TemplateResponse(request, "problems/secret-key.html", {"site_key": problem.problemsite.url_key})
...
def problem_site_tags(request, problem):
    algorithm_tag_proposals = AggregatedAlgorithmTagProposal.objects.all().filter(problem=problem).order_by("-amount")[:25]
    difficulty_tag_proposals = AggregatedDifficultyTagProposal.objects.all().filter(problem=problem).order_by("-amount")[:25]
...
def _prepare_changed_package(request, form, archive, package_name):
    ...
    if uploaded_file.name != os.path.basename(file_path):
        form.add_error(None, _("Original and replacement files must have the same name"))
        return None, None
    ...
    shutil.make_archive(base_name=package_archive_name, format="zip", root_dir=extraction_dir)
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            if "upload_button" in request.POST:
                ...
                package_archive, tmpdir = _prepare_changed_package(request, form, archive, package_name)
                if package_archive is not None:
                    package_file = File(open(package_archive, "rb"), os.path.basename(package_archive))
