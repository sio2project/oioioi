...
from oioioi.contests.models import LimitsVisibilityConfig, ProblemStatementConfig, RankingVisibilityConfig, RegistrationAvailabilityConfig
    ...
    def __init__(self, contest, existing_problem, *args, **kwargs):
        ...
        if contest and not existing_problem:
            ...
            if len(choices) >= 2:
                ...
                fields[0:0] = [("round_id", forms.ChoiceField(choices=choices, label=_("Round")))]
...
class LimitsVisibilityConfigForm(forms.ModelForm):
    class Meta:
        fields = "__all__"
...
class ProblemsetSourceForm(forms.Form):
    url_key = forms.CharField(
        label=_("Enter problem's secret key"),
        help_text=_(
            "Please note that the time and memory limits will be copied\
...
def _localized_formset_get_initial(localized_objects):
    return [{"language": lang[0]} for lang in settings.LANGUAGES if not localized_objects.filter(language=lang[0]).exists()]
