...
def _testspackage_date_name_generator(obj):
    ...
    return _("Publish tests for %(problem_name)s (%(package_name)s)") % {
        "problem_name": obj.problem.short_name,
...
class TestsPackage(models.Model):
    ...
    name = models.CharField(
        ...
        help_text=_("File name can only contain letters, digits, - and _. It should not contain file extension such as .zip, .tgz, etc."),
        validators=[
            RegexValidator(
                r"^[0-9a-zA-Z\-_]+$",
    ...
    publish_date = models.DateTimeField(
        ...
        help_text=_("If the date is left blank, the package will never be visible for participants of the contest."),
    )
        ...
        with tempfile.NamedTemporaryFile(delete=False) as f:
            ...
            for test in instance.tests.all():
                for test_file in [test.input_file, test.output_file]:
                    arch_path = strip_num_or_hash(os.path.basename(test_file.file.name))
