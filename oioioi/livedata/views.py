...
def livedata_tasks_view(request, round_id):
    ...
    pis = (
        ...
        .annotate(problem_localized_name=Subquery(ProblemName.objects.filter(problem=OuterRef("problem__pk"), language=get_language()).values("name")))
    )
    problem_localized_name = F("problem_localized_name")

    return [{"id": pi.id, "shortName": pi.short_name, "name": pi.problem.name} for pi in pis.order_by(problem_localized_name.asc(nulls_first=True))]
...
def livedata_events_view(request, round_id):
    ...
    if (is_contest_admin(request) or is_contest_observer(request)) and "from" in request.GET:
        ...
        start_time = datetime.datetime.utcfromtimestamp(int(request.GET["from"])).replace(tzinfo=datetime.UTC)
    ...
    return [
    ...
    ] + [
        {
            ...
            "result": report.score_report.status if freeze_time is None or report.submission.date < freeze_time else RESULT_FOR_FROZEN_SUBMISSION,
        }
