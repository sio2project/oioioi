...
class TestLivedata(TestCase):
    ...
    def test_cache_unless_admin_or_observer(self):
        ...
        self.assertEqual(request_first.content, request_cached.content, "Cached response differs")
        ...
        request_cached = self.client.get(url)
        self.assertEqual(request_cached.content.decode(request_cached.charset), magic_string, "Response from cache does not contain expected cached data")
        self.assertTrue(self.client.login(username="test_admin"))
        request_admin = self.client.get(url)
        self.assertNotEqual(request_admin.content.decode(request_admin.charset), magic_string, "Admin should not be receiving a response from cache")
