...
class OISubmitSubmissionForm(SubmissionForm):
    ...
    def clean(self, **kwargs):
        ...
        try:
            pi = ProblemInstance.objects.filter(contest=self.request.contest).get(short_name=self.cleaned_data["problem_shortname"])
            cleaned_data["problem_instance_id"] = pi.id
            del cleaned_data["problem_shortname"]
        except ProblemInstance.DoesNotExist:
            self._errors["problem_shortname"] = self.error_class([_("Invalid problem shortname")])
