...
class OISubmitFileMixin:
    def submit_file(
        ...
        if isinstance(localtime, datetime):
            localtime_str = localtime.astimezone(get_current_timezone()).strftime("%Y-%m-%d %H:%M:%S")
        else:
        ...
        if siotime is None:
            ...
            siotime_str = siotime.astimezone(get_current_timezone()).strftime("%Y-%m-%d %H:%M:%S")
        else:
            siotime_str = siotime
    ...
    def _assertSuspected(self, response, submission_number, suspected_number, reason):
        ...
        self.assertEqual(suspected_number + 1, Submission.objects.filter(kind="SUSPECTED").count())
    ...
    def _assertNotSuspected(self, response, submission_number, suspected_number):
        ...
        self.assertEqual(suspected_number, Submission.objects.filter(kind="SUSPECTED").count())
...
class TestOISubmitSubmission(TestCase, OISubmitFileMixin):
    ...
    def test_simple_submission(self):
        ...
        with fake_time(dt):
            submission_number = Submission.objects.all().count()
            suspected_number = Submission.objects.filter(kind="SUSPECTED").count()
            response = self.submit_file(contest, pi.short_name, dt, dt + timedelta(seconds=25))
            self._assertNotSuspected(response, submission_number, suspected_number)
        ...
        with fake_time(dt):
            submission_number = Submission.objects.all().count()
            suspected_number = Submission.objects.filter(kind="SUSPECTED").count()
            response = self.submit_file(contest, pi.short_name, dt, dt + timedelta(seconds=31))
            self._assertSuspected(
                response,
                submission_number,
                suspected_number,
        ...
        with fake_time(dt):
            submission_number = Submission.objects.all().count()
            suspected_number = Submission.objects.filter(kind="SUSPECTED").count()
            response = self.submit_file(contest, pi.short_name, dt + timedelta(seconds=31))
            self._assertSuspected(
                response,
                submission_number,
                suspected_number,
    ...
    def test_submission_form(self):
        ...
        self._assertFormError(response, submission_number, INCORRECT_FORM_COMMENTS["magickey"])
        response = self.submit_file(contest, pi.short_name, localtime="wrong")
        self._assertFormError(response, submission_number, INCORRECT_FORM_COMMENTS["localtime"])
        response = self.submit_file(contest, pi.short_name, siotime="wrong")
        self._assertFormError(response, submission_number, INCORRECT_FORM_COMMENTS["siotime"])
        response = self.submit_file(contest, "xx")
        self._assertFormError(response, submission_number, INCORRECT_FORM_COMMENTS["problem_shortname"])
