...
class ScoresRevealContestControllerMixin(object):
    ...
    def can_see_submission_score(self, request, submission):
        return super(ScoresRevealContestControllerMixin, self).can_see_submission_score(
            request, submission
    ...
    def is_scores_reveals_limit_reached(self, user, problem_instance):
        return self.get_revealed_submissions(
            user, problem_instance
        ).count() >= self.get_scores_reveals_limit(problem_instance)
    ...
    def can_reveal(self, request, submission):
        ...
        if super(ScoresRevealContestControllerMixin, self).can_see_submission_score(
            request, submission
        ):
    ...
    def is_scores_reveals_disabled(self, request, submission):
        ...
        return rtimes.is_past(
            submission.date + timedelta(minutes=disable_time)
        ) and rtimes.is_active(request.timestamp)

    def render_submission_footer(self, request, submission):
        super_footer = super(
            ScoresRevealContestControllerMixin, self
        ).render_submission_footer(request, submission)

        if (
            not has_scores_reveal(submission.problem_instance)
            or submission.kind != 'NORMAL'
            or submission.user is None
        ):
            return super_footer
