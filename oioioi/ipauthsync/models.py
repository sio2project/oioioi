...
class IpAuthSyncConfigManager(models.Manager):
    def get_active(self, timestamp):
        return self.get_queryset().filter(start_date__lte=timestamp, end_date__gte=timestamp, enabled=True)


@date_registry.register("start_date", name_generator=(lambda obj: _("Enable IP authentication sync")))
@date_registry.register("end_date", name_generator=(lambda obj: _("Disable IP authentication sync")))
