...
class Command(BaseCommand):
    ...
    def handle_config(self, config):
        ...
        for region in config.contest.regions.all():
            try:
                db = MySQLdb.connect(host=region.region_server,
                ...
                cursor = db.cursor()
                cursor.execute('SELECT z.id, k.ip FROM zaw z, komp k '
                ...
                if warnings != msgs.warnings:
                    ...
                    msgs.save()
                    mail_admins("ipauthsyncd: Warnings for region %s"
                            % (region.short_name,), warnings)
                if mapping != msgs.mapping:
                    msgs.mapping = mapping
                    msgs.save()
                    mail_admins("ipauthsyncd: Mapping for region %s"
                            % (region.short_name,), mapping)

                if region.short_name in self.failing_regions:
                    self.failing_regions.remove(region.short_name)
                    mail_admins("ipauthsyncd: Sync now OK for region %s"
                            % (region.short_name,),
    ...
    def handle(self, *args, **options):
        ...
        while True:
            configs = IpAuthSyncConfig.objects.get_active(timezone.now()) \
