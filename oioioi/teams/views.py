...
def team_members_names(request, team):
    return [team.contest.controller.get_user_public_name(request, membership.user) for membership in team.members.all()]
...
def teams_list(request):
    teams = [{"name": team.name, "members": team_members_names(request, team)} for team in Team.objects.filter(contest=request.contest)]
...
def create_team_view(request):
    if request.method == "POST":
        ...
        if team_create_form.is_valid():
            ...
            membership.save()
            return HttpResponseRedirect(reverse("team_view", kwargs={"contest_id": request.contest.id}))
    else:
        team_create_form = CreateTeamForm()
    return TemplateResponse(request, "teams/create-team.html", {"form": team_create_form})
...
def delete_team_view(request):
    tm = get_object_or_404(TeamMembership, user=request.user, team__contest=request.contest)
    team = tm.team
    user = team.user
    tm.delete()
    team.delete()
