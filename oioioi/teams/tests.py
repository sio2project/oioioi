...
class TestTeamsViews(TestCase, SubmitFileMixin):
    ...
    def test_views(self):
        ...
        with fake_time(timestamp):
            ...
            response = self.client.get(reverse("team_view", kwargs={"contest_id": contest.id}), follow=True)
...
class TestTeamSubmit(TestCase, SubmitFileMixin):
    ...
    def test_submit_file_with_team(self):
        ...
        self.assertContains(response, "You can't submit a solution for another team!", html=True)
...
class TestTeamsListView(TestCase):
    ...
    def test_list(self):
        ...
        response = self.client.get(reverse("teams_list", kwargs={"contest_id": contest.id}), follow=True)
...
class TestTeamMembership(TestCase):
    ...
    def test_max_one_team_per_user(self):
        ...
        with self.assertRaisesMessage(ValidationError, "The user is already in another team"):
