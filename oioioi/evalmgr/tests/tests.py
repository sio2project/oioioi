...
class TestRemoteJobs(TestCase):
    ...
    remote_wrong_source_file = "/test_worker_manager/" + _uuid() + "wrong_add_solution.c"
    ...
    @override_settings(SIOWORKERS_BACKEND="oioioi.evalmgr.tests.tests.SioworkersBackend")
...
class TestErrorBehavior(TestCase):
    ...
    corrupted_error_handler = [("Call police", "oioioi.evalmgr.tests.tests.corrupted_police_handler")]
...
class TestAsyncJobs(TestCase):
    ...
    def _prepare(self):
        ...
        env = create_environ()
        env.setdefault("recipe", []).append(("transfer", "oioioi.evalmgr.tests.tests._call_transfer"))
        env["resumed"] = False
        return env
...
class TestViews(TestCase):
    ...
    def _get_admin_site(self):
        ...
        show_response = self.client.get(reverse("oioioiadmin:evalmgr_queuedjob_changelist"))
