...
def get_times_status(request, response):
    ...
    if timestamp and contest:
        ...
        next_rounds_times = [(rt, round) for (rt, round) in rtimes if rt.is_future(timestamp)]
        next_rounds_times.sort(key=lambda rt_round: rt_round[0].get_start())
        current_rounds_times = [(rt, round) for (rt, round) in rtimes if rt.is_active(timestamp) and rt.get_end()]
        current_rounds_times.sort(key=lambda rt_round1: rt_round1[0].get_end())

    if current_rounds_times:
        response["round_start_date"] = time.mktime((timezone.localtime(current_rounds_times[0][0].get_start())).timetuple())
        response["round_end_date"] = time.mktime((timezone.localtime(current_rounds_times[0][0].get_end())).timetuple())
        response["round_name"] = current_rounds_times[0][1].name
    elif next_rounds_times:
        response["round_start_date"] = time.mktime((timezone.localtime(next_rounds_times[0][0].get_start())).timetuple())
    ...
    if request.method == "POST":
        ...
        if "reset-button" in request.POST:
            ...
            if current_admin_time.year >= 1900:
                ...
                request.session["admin_time"] = current_admin_time.replace(tzinfo=local_tz).astimezone(pytz.utc).isoformat()
            else:
                messages.error(request, _("Date has to be after 1900. Admin-time was not set."))
            return safe_redirect(request, next_page)
    raise Http404
