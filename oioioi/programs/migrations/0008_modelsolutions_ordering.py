...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        ...
        def modelsolutionssort_key(msol):
            return (msol.problem,
                {
                    'NORMAL': 0,
                    'SLOW': 1,
                ...
                naturalsort_key(msol.name[:msol.name.index(".")]))

        for order, msol in enumerate(sorted(orm.ModelSolution.objects.all(),
                                            key=modelsolutionssort_key)):
            msol.order_key = order
            msol.save()
