...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        for modelname in ['programs.TestReport', 'programs.GroupReport']:
            count = orm[modelname].objects.count()
            if count == 0:
                continue
            print "Migrating %d instances of %s:" % (count, modelname)
            num = 0
            update_every = max(1, count / 20)
            pb = ProgressBar(count)
            for instance in orm[modelname].objects.all():
                if num % update_every == 0:
                    pb.update(num)
                # Score deserialize and serialize will be called on save().
                instance.save()
                num += 1
