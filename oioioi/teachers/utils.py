...
def is_user_already_in_contest(user, contest):
    ...
    if user.participant_set.filter(contest=contest) or (teacher and teacher.contestteacher_set.filter(contest=contest)):
        return True
...
def validate_can_add_user_to_contest(user, contest, member_type):
    ...
    if not is_user_already_in_contest(user, contest):
        if member_type == "pupil":
            exists = len(Participant.objects.filter(contest=contest, user=user)) > 0
        elif member_type == "teacher":
            if teacher := get_user_teacher_obj(user):
                exists = len(ContestTeacher.objects.filter(contest=contest, teacher=teacher)) > 0
            else:
...
def add_user_to_contest_as(user, contest, member_type):
    ...
    if member_type == "pupil":
        ...
        teacher = get_user_teacher_obj(user)
        ContestTeacher.objects.get_or_create(contest=contest, teacher=teacher)
