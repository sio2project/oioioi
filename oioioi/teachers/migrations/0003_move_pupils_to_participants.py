...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        for p in orm.Pupil.objects.all():
            orm['participants.Participant'].objects \
                .get_or_create(contest=p.contest, user=p.user)
        orm.Pupil.objects.all().delete()

    def backwards(self, orm):
        qs = orm['participants.Participant'].objects \
            .filter(contest__controller_name=
                'oioioi.teachers.controllers.TeacherContestController')

        for p in qs:
            orm.Pupil.objects.get_or_create(contest=p.contest, user=p.user)
    ...
    models = {
        ...
        'teachers.pupil': {
            'Meta': {'unique_together': "(('contest', 'user'),)", 'object_name': 'Pupil'},
            'contest': ('django.db.models.fields.related.ForeignKey', [], {'to': "orm['contests.Contest']"}),
            'id': ('django.db.models.fields.AutoField', [], {'primary_key': 'True'}),
            'user': ('django.db.models.fields.related.ForeignKey', [], {'to': "orm['auth.User']"})
        },
    ...
    complete_apps = ['participants', 'teachers']
    symmetrical = True
