...
def send_request_email(request, teacher, message):
    context = {
        "teacher": teacher,
        "accept_link": request.build_absolute_uri(reverse("accept_teacher", kwargs={"user_id": teacher.user_id})),
...
def send_acceptance_email(request, teacher):
    context = {
        "teacher": teacher,
        "new_contest_link": request.build_absolute_uri(reverse("oioioiadmin:contests_contest_add")),
    }
...
@account_menu_registry.register_decorator(_("Request teacher account"), lambda request: reverse("add_teacher"), order=100)
...
def accept_teacher_view(request, user_id):
    ...
    if teacher.is_active:
        ...
        choice = confirmation_view(request, "teachers/confirm_add_teacher.html", {"teacher": teacher})
        ...
        if choice:
            ...
            messages.success(request, _("Successfully accepted and emailed the new teacher."))
...
def members_view(request, member_type):
    registration_config, created = RegistrationConfig.objects.get_or_create(contest=request.contest)
    ...
    other_contests = Contest.objects.filter(contestteacher__teacher__user=request.user).exclude(id=request.contest.id)
...
def activate_view(request, key):
    ...
    if not request.method == "POST" or "register_as" not in request.POST:
        ...
        if register_as == "pupil":
            _p, created = Participant.objects.get_or_create(contest=request.contest, user=request.user)
        elif is_teacher_registration and register_as == "teacher":
            teacher_obj = get_object_or_404(Teacher, user=request.user)
            _ct, created = ContestTeacher.objects.get_or_create(contest=request.contest, teacher=teacher_obj)
        else:
...
def delete_members_view(request, member_type):
    if member_type == "teacher":
        ContestTeacher.objects.filter(contest=request.contest, teacher__user_id__in=request.POST.getlist("member")).delete()
    elif member_type == "pupil":
        Participant.objects.filter(contest=request.contest, user_id__in=request.POST.getlist("member")).delete()
    else:
        raise Http404
...
def bulk_add_members_view(request, other_contest_id):
    ...
    for ct in ContestTeacher.objects.filter(contest=other_contest):
        ContestTeacher.objects.get_or_create(contest=request.contest, teacher=ct.teacher)
...
if "oioioi.simpleui" in settings.INSTALLED_APPS:
    ...
    def teacher_dashboard_view(request):
        ...
        for contest in contests:
            if isinstance(contest["contest_controller"], TeacherContestController):
                contest["dashboard_url"] = reverse("teacher_contest_dashboard", kwargs={"contest_id": contest["id"]})
    ...
    @register_contest_dashboard_view(order=50, condition=(contest_exists & is_contest_admin & ~is_superuser))
    ...
    try:
        if form.is_valid():
            ...
            try:
                ...
                messages.success(request, _("User '%(user)s' successfully added as a '%(member_type)s'.") % {"user": user, "member_type": member_type})
