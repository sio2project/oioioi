...
class RankingController(RegisteredSubclassesBase, ObjectWithMixins):
    ...
    def get_rendered_ranking(self, request, partial_key):
        ...
        try:
            ...
            if page_nr == 1:
                return mark_safe(render_to_string(
                    "rankings/generating_ranking.html"))
        ...
        return mark_safe(render_to_string("rankings/rendered_ranking.html",
                context))
...
class DefaultRankingController(RankingController):
    ...
    def _is_problem_statement_visible(self, key, pi, timestamp):
        ...
        return ccontroller.can_see_problem(context, pi) and \
               ccontroller.can_see_statement(context, pi)

    def _get_pis_with_visibility(self, key, pis):
        now = timezone.now()
        return [(pi, self._is_problem_statement_visible(key, pi, now))
                for pi in pis]
