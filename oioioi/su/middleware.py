...
from oioioi.su import BLOCKED_URL_NAMESPACES, BLOCKED_URLS, SU_BACKEND_SESSION_KEY, SU_ORIGINAL_CONTEST, SU_REAL_USER_IS_SUPERUSER, SU_UID_SESSION_KEY
    ...
    def _process_request(self, request):
        if not hasattr(request, "user"):
            raise ImproperlyConfigured(
                "The SuAuthenticationMiddleware requires the 'django.contrib.auth.middleware.AuthenticationMiddleware' earlier in MIDDLEWARE."
            )
        ...
        if SU_UID_SESSION_KEY in request.session:
            ...
            if not request.session.get(SU_REAL_USER_IS_SUPERUSER, True):
                ...
                if not is_su_reset_url and not nonoioioi_namespace and (contest_id is None or contest_id != original_contest_id):
                ...
                if not is_su_reset_url and getattr(settings, "ALLOW_ONLY_GET_FOR_SU_CONTEST_ADMINS", True) and request.method != "GET":
                    return redirect("su_method_not_allowed", contest_id=original_contest_id)


class SuFirstTimeRedirectionMiddleware:
