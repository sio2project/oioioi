...
class TestSwitchingUsers(TestCase):
    ...
    def test_switching_users(self):
        ...
        self.assertEqual("django.contrib.auth.backends.ModelBackend", session[SU_BACKEND_SESSION_KEY])
    ...
    def test_inheriting_backend(self):
        ...
        request.user = get_user(request, test_user.id, "oioioi.base.tests.IgnorePasswordAuthBackend")
        request.session = {}
        su_to_user(request, test_user2)

        self.assertEqual("test_user2", request.user.username)
        self.assertEqual("oioioi.base.tests.IgnorePasswordAuthBackend", request.user.backend)
...
class TestContestAdminsSu(TestCase):
    ...
    def test_users_list(self):
        ...
        response = self.client.get(reverse("get_suable_users", kwargs={"contest_id": contest.id}), {"substr": "te"})
        response = response.json()
        self.assertListEqual(
            [
                "test_user (Test User)",
    ...
    def test_su(self):
        ...
        for url in urls:
            ...
            self.assertEqual(302, response.status_code)
            self.assertEqual(reverse("su_url_not_allowed", kwargs={"contest_id": contest.id}), response.url)
    ...
    def _test_post(self, can_post):
        ...
        if can_post:
            ...
            self.assertEqual(302, response.status_code)
            self.assertEqual(reverse("su_method_not_allowed", kwargs={"contest_id": contest.id}), response.url)
    ...
    def test_blocked_accounts(self):
        ...
        for username in ["test_admin", "test_contest_admin"]:
            response = self.client.get(reverse("get_suable_users", kwargs={"contest_id": contest.id}), {"substr": username[:2]})
            response = response.json()
