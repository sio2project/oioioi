$(function() {
    var rowTemplate = '<tr><td class="color-cell"></td><td class="team-cell">'
            + '__TEAM__</td><td class="button-cell"></td></tr>';
    var deliveredButtonTemplate = '<button class="btn btn-primary btn-small" '
            + 'data-loading-text="' + gettext("Delivering...") + '">'
            + gettext("Delivered") + '</button>';
    var notDeliveredButtonTemplate = '<button class="btn btn-danger btn-small"'
            + ' data-loading-text="' + gettext("Taking back...") + '">'
            + gettext("Not delivered") + '</button>';
    var firstAcceptedTemplate = '<i class="icon-star"></i> <b>'
            + gettext("First to solve") + '</b> ';
    var lastRequestId = -1;
    var fetchInterval = 5000;
    var fadeInterval = 1000;
    var errorDiv = $('.top-row').find('div.alert-error');
    var createRow = function(color, team, id, isFirst, problemName) {
        ...
        if (color === null) {
            colorCellText += ' <small class="no-color">' + gettext("No color")
                + '</small>';
        } else {
