...
class TestBalloons(TestCase):
    ...
    def test_balloons_link_and_cookie(self):
        ...
        regenerate_url = reverse("balloons_access_regenerate", kwargs={"contest_id": self.contest.id})
        ...
        self.assertTrue(response["Location"].endswith(reverse("oioioiadmin:contests_contest_change", args=[quote(self.contest.id)])))
    ...
    def test_getting_new_balloon_requests(self):
        ...
        requests = [BalloonDelivery(user=user, problem_instance=self.pi) for user in users]
