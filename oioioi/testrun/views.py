...
def testrun_submit_view(request, problem_instance_id=None):
    ...
    submissions_left = {pi.id: pi.controller.get_submissions_left(request, pi, "TESTRUN") for pi in problem_instances}

    # Testrun time limits in seconds.
    time_limits = {pi.id: float(pi.controller.get_test_run_time_limit(pi)) / 1000 for pi in problem_instances}

    # Testrun memory limits in MB.
    memory_limits = {pi.id: pi.controller.get_test_run_memory_limit(pi) / 1024 for pi in problem_instances}
...
def get_testrun_report_or_404(request, submission, testrun_report_id=None, model=TestRunReport):
...
def show_input_file_view(request, submission_id):
    submission = get_submission_or_error(request, submission_id, TestRunProgramSubmission)
