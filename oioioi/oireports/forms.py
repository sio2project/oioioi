...
def _last_finished_round_id(request):
    past_rounds = [rt for rt in rounds_times(request, request.contest or None).items() if rt[1].is_past(request.timestamp)]
    if not past_rounds:
        return None
...
class OIReportForm(forms.Form):
    ...
    def __init__(self, request, *args, **kwargs):
        ...
        self.fields["report_round"] = forms.ChoiceField(choices=rounds, label=_("Round"), initial=last_finished_round_id)
        self.fields["report_region"] = forms.ChoiceField(choices=regions, label=_("Region"))

        self.fields["single_report_user"].hints_url = reverse("get_report_users", kwargs={"contest_id": request.contest.id})
        self.fields["single_report_user"].queryset = request.contest.controller.registration_controller().filter_participants(User.objects.all())
    ...
    def clean(self):
        ...
        if cleaned_data["is_single_report"]:
            ...
            if report_user is None:
                self._errors["single_report_user"] = self.error_class([_("No user specified.")])
    ...
    def get_testgroups(self, request):
        ...
        for field_name in self.fields:
            ...
            if match:
                ...
                problem_instance = ProblemInstance.objects.get(short_name=match.group(1), round__contest=request.contest)
                if groups:
