...
from oioioi.interactive.models import InteractiveTaskInfo, Interactor
...
def _determine_encoding(title, file):
    r = re.search(rb"\\documentclass\[(.+)\]{sinol}", file)
    encoding = "latin2"
...
class SinolPackage:
    ...
    def __init__(self, path, original_filename=None):
        ...
        self.restrict_html = settings.SINOLPACK_RESTRICT_HTML and not settings.USE_SINOLPACK_MAKEFILES
    ...
    def _find_and_compile(self, suffix, command=None, cwd=None, log_on_failure=True, out_name=None):
        if not command:
            command = suffix
        if self.use_make:
    ...
    def _override_compiler(self, prefix, lang, compilation_job):
        ...
        name_map = {"c": "C", "cpp": "C++", "pas": "Pascal", "java": "Java", "py": "Python"}
    ...
    def _run_compilation_job(self, ext, ft_source_name, out_name):
        ...
        add_extra_files(compilation_job, self.problem, additional_args=self.extra_compilation_args)
    ...
    def _ensure_compilation_success(self, filename, new_env):
        ...
        if compilation_result != "OK":
            ...
            logger.warning("%s: compiler output: %r", self.filename, compilation_message)

            raise ProblemPackageError(
                _("Compilation of file %(filename)s failed. Compiler output: %(output)s") % {"filename": filename, "output": compilation_message}
            )
    ...
    def _create_problem_instance(self):
        ...
        return Problem.create(
            ...
            visibility=(Problem.VISIBILITY_PUBLIC if author is None else self.env.get("visibility", Problem.VISIBILITY_FRIENDS)),
            author=author,
        )
    ...
    def _describe_processing_error(func):
        ...
        def wrapper(*args, **kwargs):
            try:
                ...
                error = PackageProcessingError(func.__name__, func.__doc__.split("\n\n")[0])
    ...
    def _process_config_yml(self):
        ...
        if os.path.isfile(config_file):
            ...
            instance.config = open(config_file, encoding="utf-8").read()
        else:
            instance.config = ""
        instance.save()
    ...
    def _validate_package_naming_consistency(self):
        ...
        if len(detected_prefixes) > 1:
            ...
            if detected != self.short_name:
                raise ProblemPackageError(
                    _("Package naming mismatch. Folder name: '%(folder)s', Files use prefix: '%(prefix)s'. Make sure they are consistent.")
    ...
    def _detect_full_name(self):
        ...
        if os.path.isfile(source):
            ...
            r = re.search(rb"^[^%]*\\title{(.+)}", text, re.MULTILINE)
            if r is not None:
    ...
    def _detect_library(self):
        ...
        if "library" in self.config and self.config["library"]:
            instance, _created = LibraryProblemData.objects.get_or_create(problem=self.problem)
    ...
    def _process_extra_files(self):
        ...
        if not_found:
            raise ProblemPackageError(_("Expected extra files %r not found in prog/") % (not_found))
    ...
    def _extract_makefiles(self):
        sinol_makefiles_tgz = os.path.join(os.path.dirname(__file__), "files", "sinol-makefiles.tgz")
    ...
    def _save_prog_dir(self):
        ...
        archive = shutil.make_archive(os.path.join(self.rootdir, archive_name), format="zip", root_dir=prog_dir)
        self.prog_archive = get_client().put_file(_make_filename_in_job_dir(self.env, archive), archive)
        ...
        for lang in lang_prefs:
            ...
            if os.path.isfile(htmlzipfile):
                if self._html_disallowed():
                    raise ProblemPackageError(_("You cannot upload package with problem statement in HTML. Try again using PDF format."))

                self._force_index_encoding(htmlzipfile)
                statement = ProblemStatement(problem=self.problem, language=lang[1:])
                statement.content.save(self.short_name + lang + ".html.zip", File(open(htmlzipfile, "rb")))

            pdffile = os.path.join(docdir, self.short_name + "zad" + lang + ".pdf")

            if os.path.isfile(pdffile):
                statement = ProblemStatement(problem=self.problem, language=lang[1:])
                statement.content.save(self.short_name + lang + ".pdf", File(open(pdffile, "rb")))
    ...
    def _force_index_encoding(self, htmlzipfile):
        ...
        """
        with zipfile.ZipFile(htmlzipfile, "r") as archive, archive.open("index.html") as index:
            data = index.read()
            ...
            # so index.html has to be translated to utf-8.
                ...
                try:
                    ...
                    raise ProblemPackageError(_("index.html has to be utf8 or iso8859-2 encoded"))
    ...
    def _compile_latex_docs(self, docdir):
        ...
        src_fancyheadings = os.path.join(os.path.dirname(__file__), "files", "fancyheadings.sty")
        ...
        # already exist (-k).
        sinol_cls_tgz = os.path.join(os.path.dirname(__file__), "files", "sinol-cls.tgz")
        execute(["tar", "-C", docdir, "--skip-old-files", "-zxf", sinol_cls_tgz], cwd=docdir)

        try:
            execute("make", cwd=docdir)
        except ExecuteError:
            logger.warning("%s: failed to compile statement", self.filename, exc_info=True)
    ...
    def _detect_statement_memory_limit(self):
        ...
        if os.path.isfile(source):
            with open(source, "rb") as f:
                text = f.read()
            r = re.search(rb"^[^%]*\\RAM{(\d+)}", text, re.MULTILINE)
            if r is not None:
                try:
                    value = int(r.group(1))
        ...
        if env and not self.use_make:
            ...
            for test_name, job in jobs.items():
                if job["result_code"] != "OK":
                    raise ProblemPackageError(
                        _("Inwer failed on test %(test)s. Inwer output %(output)s") % {"test": test_name, "output": "\n".join(job["stdout"])}
                    )
    ...
    def _validate_tests(self, created_tests):
        ...
        for instance in created_tests:
            if self.task_type == TaskType.STANDARD and not instance.output_file:
                raise ProblemPackageError(_("Missing out file for test %s") % instance.name)
            try:
                instance.full_clean()
    ...
    def _delete_non_existing_tests(self, created_tests):
        for test in Test.objects.filter(problem_instance=self.main_problem_instance).exclude(id__in=[instance.id for instance in created_tests]):
    ...
    def _process_test(
        ...
        instance, created = Test.objects.get_or_create(problem_instance=self.main_problem_instance, name=name)
    ...
    def _compute_scores_automatically(self, scored_groups, total_score):
        ...
        extra_score_groups = sorted(scored_groups, key=naturalsort_key)[num_groups - (total_score - num_groups * group_score) :]
        for group in scored_groups:
            score = group_score
    ...
    def _assign_scores(self, scored_groups, total_score_if_auto):
        ...
        if group_scores_from_config:
            ...
            scores = self._compute_scores_automatically(scored_groups, total_score_if_auto)

        Test.objects.filter(problem_instance=self.main_problem_instance).update(max_score=0)
    ...
    def _set_memory_limit_overrides(self, lang, rules):
        ...
        if "memory_limit" in rules:
            tests = Test.objects.filter(problem_instance=self.main_problem_instance)
            for test in tests:
                LanguageOverrideForTest.objects.filter(test=test, language=lang).update(memory_limit=rules["memory_limit"])
        elif "memory_limits" in rules:
            for group, limit in rules["memory_limits"].items():
                tests = Test.objects.filter(problem_instance=self.main_problem_instance, group=group)
                for test in tests:
                    LanguageOverrideForTest.objects.filter(test=test, language=lang).update(memory_limit=limit)
    ...
    def _set_time_limit_overrides(self, lang, rules):
        ...
        if "time_limit" in rules:
            tests = Test.objects.filter(problem_instance=self.main_problem_instance)
            for test in tests:
                LanguageOverrideForTest.objects.filter(test=test, language=lang).update(time_limit=rules["time_limit"])
        elif "time_limits" in rules:
            for group, limit in rules["time_limits"].items():
                tests = Test.objects.filter(problem_instance=self.main_problem_instance, group=group)
                for test in tests:
                    LanguageOverrideForTest.objects.filter(test=test, language=lang).update(time_limit=limit)
    ...
    def _process_model_solutions(self):
        ...
        for order, (short_kind, name, path) in enumerate(sorted(progs, key=modelsolutionssort_key)):
    ...
    def _get_model_solutions_sources(self):
        ...
        progs = [
            (x[0].group(1), x[1], x[2])
            for x in ((names_re.match(name), name, os.path.join(progdir, name)) for name in os.listdir(progdir))
    ...
    def _process_attachments(self):
        ...
        if not os.path.isdir(attachments_dir):
            return
        attachments = [attachment for attachment in os.listdir(attachments_dir) if os.path.isfile(os.path.join(attachments_dir, attachment))]
    ...
    def _save_original_package(self):
        ...
        """
        original_package, created = OriginalPackage.objects.get_or_create(problem=self.problem)
...
class SinolPackageBackend(ProblemPackageBackend):
    ...
    def unpack(self, env):
        ...
        with tempfile.NamedTemporaryFile() as tmpfile:
            ...
            tmpfile.flush()
            problem = self.package_class(tmpfile.name, package.package_file.name).unpack(env, package)
