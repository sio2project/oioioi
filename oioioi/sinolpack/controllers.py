...
class SinolProblemController(ProgrammingProblemController):
    ...
    def mixins_for_admin(self):
        return super(SinolProblemController, self).mixins_for_admin() + (SinolpackProblemAdminMixin,)
...
class SinolInteractiveProblemController(InteractiveProblemController):
    ...
    def fill_evaluation_environ(self, environ, submission, **kwargs):
        super(SinolInteractiveProblemController, self).fill_evaluation_environ(environ, submission, **kwargs)
        add_extra_files(environ, self.problem)

    def mixins_for_admin(self):
        return super(SinolInteractiveProblemController, self).mixins_for_admin() + (SinolpackProblemAdminMixin,)
