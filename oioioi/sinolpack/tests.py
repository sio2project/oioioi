...
class TestSinolPackage(TestCase, TestStreamingMixin):
    ...
    def test_single_file_replacement(self):
        ...
        url = reverse("problem_site", kwargs={"site_key": site_key}) + "?key=manage_files_problem_package"
    ...
    def test_title_translations_in_config_yml(self):
        ...
        self.assertTrue(problem_names.filter(name="Problem with translations", language="en").exists())
        self.assertTrue(problem_names.filter(name="Zadanie z tłumaczeniami", language="pl").exists())
    ...
    def test_latin2_title_from_doc(self):
        ...
        call_command("addproblem", filename)
        problem = Problem.objects.get()
        self.assertEqual(problem.name, "Łąka")
    ...
    def test_utf8_title_from_config(self):
        ...
        call_command("addproblem", filename)
        problem = Problem.objects.get()
        self.assertEqual(problem.name, "ĄąĆćĘęŁłÓóŚśŻżŹź")
    ...
    def test_huge_unpack_update(self):
        ...
        url = reverse("add_or_update_problem") + "?" + urllib.parse.urlencode({"problem": problem.id})
    ...
    def _check_full_package(self, problem, doc=True):
        ...
        if doc:
            self.assertEqual(problem.name, "Sumżyce")
        ...
        model_solutions = ModelSolution.objects.filter(problem=problem).order_by("order_key")
    ...
    def _check_interactive_package(self, problem):
        ...
        self.assertEqual(problem.name, "arc")
        ...
        t1a = tests.get(name="1a")
        self.assertEqual(t1a.input_file.read(), b"0\n-435634223 1 30 23 130 0 -324556462\n")
        self.assertEqual(
...
22\n107\n84\n112\n92\n63\n125\n82\n1\n""",
        ...
        self.assertEqual(t2a.input_file.read(), b"0\n-435634223 1 14045 547 60000 0 -324556462\n")
    ...
    def test_restrict_html(self):
        self.assertTrue(self.client.login(username="test_user"))
        filename = get_test_filename("test_simple_package_with_malicious_html_statement.zip")
...
class TestSinolPackageInContest(TransactionTestCase, TestStreamingMixin):
    ...
    def test_upload_and_download_package(self):
        ...
        self.assertEqual(ProblemInstance.objects.get(contest=contest).submissions_limit, 123)
        ...
        url = reverse("add_or_update_problem", kwargs={"contest_id": contest.id}) + "?" + urllib.parse.urlencode({"problem": problem_instance.problem.id})
        response = self.client.get(url, follow=True)
        url = response.redirect_chain[-1][0]
        self.assertEqual(response.status_code, 200)
        self.assertIn(
...
class TestSinolPackageCreator(TestCase, TestStreamingMixin):
    ...
    def test_sinol_package_creator(self):
        problem = Problem.objects.get()
        self.assertTrue(self.client.login(username="test_admin"))
        response = self.client.get(reverse("oioioiadmin:problems_problem_download", args=(problem.id,)))
        self.assertEqual(response.status_code, 200)
...
class TestJudging(TestCase):
    ...
    def test_judging(self):
        ...
        response = self.client.post(url, {"problem_instance_id": pi_id, "file": open(filename, "rb")})
...
class TestLimits(TestCase):
    ...
    def test_time_limit(self):
        ...
        self.assertContains(
            response,
            escape("Sum of time limits for all tests is too big. It's 50s, but it shouldn't exceed 2s."),
        )
