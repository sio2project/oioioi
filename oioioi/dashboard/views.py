...
def dashboard_message_edit_view(request):
    ...
    return TemplateResponse(request, "public_message/edit.html", {"form": form, "title": _("Edit dashboard message")})
...
def dashboard_message_fragment(request):
    ...
    return render_to_string("dashboard/dashboard-message.html", context=context, request=request)
...
def top_links_fragment(request):
    ...
    return render_to_string("dashboard/dashboard-actions.html", context=context, request=request)
...
def submissions_fragment(request):
    ...
    return render_to_string("dashboard/dashboard-submissions.html", context=context, request=request)
...
@menu_registry.register_decorator(
    _("Dashboard"),
    lambda request: reverse("contest_dashboard", kwargs={"contest_id": request.contest.id}),
    order=20,
...
def public_contest_dashboard_view(request):
    ...
    if not fragments:
        fragments = [render_to_string("dashboard/dashboard-empty.html", request=request)]
    context = {
        "headers": headers,
        "fragments": fragments,
    }
