...
class TestPAScore(TestCase):
    def test_score_distribution(self):
        ...
        self.assertEqual(
            repr(dist1),
            "ScoreDistribution(10: 1, 9: 0, 8: 0, 7: 0, 6: 0, 5: 0, 4: 0, 3: 0, 2: 0, 1: 0)",
        )
    ...
    def test_pa_score(self):
        ...
        score3 = score[10] + score[10] + score[10] + score[4] + score[2] + score1 + score2

        self.assertEqual(score3, (3 * 10 + 4 + 2 + 2 * 8))
        self.assertEqual(
            repr(score3),
            "PAScore(IntegerScore(52), ScoreDistribution(10: 3, 9: 0, 8: 0, 7: 0, 6: 0, 5: 0, 4: 1, 3: 0, 2: 4, 1: 10))",
        )
        self.assertEqual(
            score3._to_repr(),
            "0000000000000000052;00003:00000:00000:00000:00000:00000:00001:00000:00004:00010",
        )
    ...
    def test_round_states(self):
        ...
        def check_round_state(date, expected):
            ...
            with fake_timezone_now(date):
                url = reverse("ranking", kwargs={"contest_id": "c", "key": A_PLUS_B_RANKING_KEY})
            ...
            self.assertEqual(expected[1], controller.can_see_source(request, not_my_submission))

            self.assertEqual(False, controller.can_see_source(request, not_last_submission))
...
class TestPARegistration(TestCase):
    ...
    def test_default_terms_accepted_phrase(self):
        ...
        self.assertContains(
            response,
            "I declare that I have read the contest rules and the technical arrangements. I fully understand them and accept them unconditionally.",
        )
...
class TestPAScorer(TestCase):
    ...
    def test_pa_test_scorer(self):
        results = list(map(utils.pa_test_scorer, *list(zip(*self.t_results_ok, strict=False))))
        self.assertEqual(self.t_expected_ok, results)

        results = list(map(utils.pa_test_scorer, *list(zip(*self.t_results_wrong, strict=False))))
...
class TestPAResults(TestCase):
    ...
    def test_pa_user_results(self):
        contest = Contest.objects.get()
        user = User.objects.get(username="test_user")
        old_results = sorted([result.score for result in UserResultForProblem.objects.filter(user=user)])
        for pi in ProblemInstance.objects.all():
            contest.controller.update_user_results(user, pi)
        new_results = sorted([result.score for result in UserResultForProblem.objects.filter(user=user)])
        self.assertEqual(old_results, new_results)


@override_settings(PROBLEM_PACKAGE_BACKENDS=("oioioi.problems.tests.DummyPackageBackend",))
class TestPADivisions(TestCase):
    ...
    def test_prolem_upload(self):
        ...
        url = reverse("add_or_update_problem", kwargs={"contest_id": contest.id}) + "?" + urllib.parse.urlencode({"key": "upload"})
        ...
        url = reverse("add_or_update_problem", kwargs={"contest_id": contest.id}) + "?" + urllib.parse.urlencode({"problem": problem.id, "key": "upload"})
        response = self.client.get(url)
