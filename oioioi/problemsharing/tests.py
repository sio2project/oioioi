...
class TestFriendshipManagement(TestCase):
    ...
    def _addToFriends(self, name):
        self.assertEqual(self.client.post(self.url, {"befriend": "", "user": name}).status_code, 200)

    def _removeFromFriends(self, id):
        self.assertEqual(self.client.post(self.url, {"unfriend": "", "id": id}).status_code, 200)
