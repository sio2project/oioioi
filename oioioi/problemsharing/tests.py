...
class TestFriendshipManagement(TestCase):
    ...
    def testAdd(self):
        ...
        self.assertEquals(len(Friendship.objects.all()), 0)
        self._addToFriends('test_user')
        self.assertEquals(len(Friendship.objects.all()), 0)
        self._addToFriends('test_user2')
        self.assertEquals(len(Friendship.objects.all()), 1)
    ...
    def testOnlyTeachersCanUseFriends(self):
        ...
        self.assertEquals(self.client.get(self.url).status_code, 403)
