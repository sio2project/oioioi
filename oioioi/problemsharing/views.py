    ...
    def post(self, request):
        ...
        if "befriend" in request.POST:
            ...
            if ctx["form"].is_valid():
                ...
                if Friendship.objects.filter(creator=request.user, receiver=receiver).exists():
            ...
            try:
                friendship = Friendship.objects.get(creator=request.user, receiver_id=request.POST.get("id"))
                friendship.delete()
...
def friend_hints_view(request):
    queryset = User.objects.filter(teacher__isnull=False).exclude(friendships_received__creator=request.user).exclude(id=request.user.id)
