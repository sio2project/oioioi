...
from oioioi.contests.utils import get_submission_message, is_contest_archived, is_contest_basicadmin
...
class QuizContestControllerMixin:
...
class QuizProblemController(ProblemController):
    ...
    def render_pictures(self, pictures):
        return (
            format_html(
                """<div class="quiz_pictures_container">
            <table>
                <tr>{}</tr>
            </table>
        </div>""",
    ...
    def add_question_to_form(self, request, form, problem_instance, question):
        ...
        if question.is_text_input:
            form.fields[field_name] = forms.CharField(label=label, required=False, strip=True)
            ...
            form.fields[field_name] = forms.ChoiceField(label=label, choices=answers, widget=forms.RadioSelect, required=False)
    ...
    def create_submission(self, request, problem_instance, form_data, **kwargs):
        ...
        with transaction.atomic():
            ...
            for question in questions:
                ...
                selected_answers = self._get_selected_answers(form_data, field_id, question)
    ...
    def _submit_answers(self, selected_answers, question, submission):
        if question.is_text_input:
            ...
            answers = {a.id: a.id in selected_answers for a in question.quizanswer_set.all()}
            for aid, selected in answers.items():
                answer = QuizAnswer.objects.get(id=aid)
                sub = QuizSubmissionAnswer.objects.create(quiz_submission=submission, answer=answer, is_selected=selected)
                sub.save()
    ...
    def render_report(self, request, report):
        ...
        question_reports = QuestionReport.objects.filter(submission_report=report).order_by("question__order")
    ...
    def render_submission(self, request, submission):
        ...
        context = {
            "submission": submission_template_context(request, submission.quizsubmission),
