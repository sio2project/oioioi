...
class SubmitQuizMixin(SubmitMixin):
    def submit_quiz(self, contest, problem_instance, answers):
        ...
        for qid in answers:
            post_data.update({"quiz_" + str(problem_instance.id) + "_q_" + str(qid): answers[qid]})
...
class TestSubmission(TestCase, SubmitQuizMixin):
    ...
    def test_simple_submission(self):
        ...
        response = self.submit_quiz(contest, problem_instance, {"1": "1", "2": ("3", "4")})
        self._assertSubmitted(contest, response)
...
class TestScore(TestCase):
    ...
    def test_multiple_choice_no_correct_answer_score(self):
        ...
        controller.judge(submission)
        submission_report = SubmissionReport.objects.get(submission=submission, status="ACTIVE")
        question_report = QuestionReport.objects.get(question=3, submission_report=submission_report)
    ...
    def test_all_answers_correct_score(self):
        ...
        controller.judge(submission)
        submission_report = SubmissionReport.objects.get(submission=submission, status="ACTIVE")
        question_report = QuestionReport.objects.get(question=1, submission_report=submission_report)
    ...
    def test_one_answer_incorrect_score(self):
        ...
        controller.judge(submission)
        submission_report = SubmissionReport.objects.get(submission=submission, status="ACTIVE")
        question_report = QuestionReport.objects.get(question=2, submission_report=submission_report)
...
class TestScoreRejudgeAfterNewQuestionsAdded(TestScore):
    def setUp(self):
        ...
        self.closed_added_question = QuizQuestion.objects.create(quiz=self.quiz, question="First added question")
        ...
        self.open_added_question = QuizQuestion.objects.create(question="Second added question", quiz=self.quiz, is_text_input=True)
        QuizAnswer.objects.create(
    ...
    def test_closed_added_question_no_given_answer_score(self):
        ...
        submission_report = SubmissionReport.objects.get(submission=self.submission, status="ACTIVE")
        question_report = QuestionReport.objects.filter(question=self.closed_added_question, submission_report=submission_report)
    ...
    def test_open_added_question_no_given_answer_score(self):
        self.controller.judge(self.submission)
        submission_report = SubmissionReport.objects.get(submission=self.submission, status="ACTIVE")
        question_report = QuestionReport.objects.filter(question=self.open_added_question, submission_report=submission_report)
...
class TestSubmissionView(TestCase):
    ...
    def test_submission_score_visible(self):
        ...
        expected_score = 50
        response = self.client.get(reverse("submission", kwargs=kwargs))
        self.assertContains(response, f"<td>{expected_score}</td>", html=True)
...
class TestQuizProblemView(TestCase):
    ...
    def test_quiz_tab_visibility(self):
        ...
        for allowed_tab, disabled_tab in zip(self.allowed_quiz_tabs, self.disabled_quiz_tabs, strict=False):
