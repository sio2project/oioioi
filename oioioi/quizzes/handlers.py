...
def score_quiz(env, **kwargs):
    ...
    for question in questions:
        score_tmp, ignore_question = _score_question(submission, submission_report, question, submission.problem_instance)
...
def _is_answer_correct(submitted_answer):
    return submitted_answer.is_selected == QuizAnswer.objects.get(pk=submitted_answer.answer.id).is_correct
...
def _score_question(submission, submission_report, question, problem_instance):
    ...
    if question.is_text_input:
        ...
        if text_answers.exists():
            ...
            award_points = any(_match_text_input(question, text_answer, answer.answer, problem_instance) for answer in correct_answers)
