...
class TestTimelineView(TestCase):
    ...
    def test_response(self):
        ...
        for round in Round.objects.filter(contest=contest.id).values():
            self.assertContains(response, round["start_date"].strftime("%Y-%m-%d %H:%M"))
...
class TestChangingDates(TestCase):
    ...
    def _send_post(
        ...
        for admin in admin_codes:
            ...
            if admin_assertin is not None:
                self.assertContains(response, admin_assertin, status_code=admin_codes[admin])

            if admin_assertnotin is not None:
                self.assertNotContains(response, admin_assertnotin, status_code=admin_codes[admin])
    ...
    def test_valid_date_change(self):
        ...
        response = self._send_post(contest, data=new_dates, admin_assertnotin="alert-danger")
    ...
    def test_valid_unset_date_change(self):
        ...
        self.assertEqual(obj.end_date, datetime.datetime(2021, 10, 10, 10, 10, tzinfo=datetime.UTC))
    ...
    def test_invalid_date_change(self):
        ...
        response = self._send_post(contest, data=new_dates, admin_assertin="alert-danger")
    ...
    def test_invalid_date_format(self):
        ...
        self._send_post(contest, data=new_dates, admin_assertin="Date format is invalid")
