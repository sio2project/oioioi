...
def score_report_is_valid(score_report):
    return score_report is not None and score_report.score is not None and score_report.max_score is not None
...
def get_round_context(request, round_pk):
    ...
    queryset = ProblemInstance.objects.filter(round=selected_round).select_related("problem")

    visible_problem_instances = [pi for pi in queryset if controller.can_see_problem(request, pi)]
    ...
    submission_counts = (
        Submission.objects.filter(problem_instance__round=selected_round, date__range=last_week)
        .values("problem_instance")
    ...
    question_counts = Message.objects.filter(round=selected_round, date__range=last_week).values("problem_instance").annotate(question_count=Count("id"))
    ...
    results = UserResultForProblem.objects.filter(problem_instance__round=selected_round, submission_report__isnull=False)
...
def contest_dashboard_view(request, round_pk=None):
    ...
    messages = messages_template_context(request, visible_messages(request))[:NUMBER_OF_RECENT_ACTIONS]
    ...
    ss = [submission_template_context(request, s) for s in queryset[:NUMBER_OF_RECENT_ACTIONS]]
    ...
    if round_pk is None and len(rtimes) > 0:
        ...
        round_pk = next(((r, rt) for r, rt in rtimes if rt.is_active(request.timestamp)), rtimes[-1])[0].pk
...
def user_dashboard_view(request):
    ...
    for contest in contests:
        scores = [result.score.to_int() for result in UserResultForContest.objects.filter(contest=contest).all() if result.score is not None]

        max_score = 0
        for problem_inst in ProblemInstance.objects.filter(contest=contest):
            user_results = UserResultForProblem.objects.filter(problem_instance=problem_inst, submission_report__isnull=False)
        ...
        contest_dict = {
            ...
            "submission_count": Submission.objects.filter(problem_instance__contest=contest).count(),
            "recent_submission_count": Submission.objects.filter(problem_instance__contest=contest, date__gte=min_date).count(),
            "recent_question_count": Message.objects.filter(contest=contest, kind="QUESTION", date__gte=min_date).count(),
            "max_score": max_score,
            "scores": scores,
            "contest_controller": contest.controller,
            "dashboard_url": reverse("simpleui_contest_dashboard", kwargs={"contest_id": contest.id}),
            "public_contest": isinstance(
        ...
        if not contest_dict["public_contest"]:
            contest_dict["user_count"] = contest.controller.registration_controller().filter_participants(User.objects.all()).count()
    ...
    if has_portal:
        context["portal_path"] = Portal.objects.filter(owner=None, link_name="default")[0].root.get_path()
...
def problem_settings(request, problem_instance_id):
    pi = get_object_or_404(ProblemInstance, id=problem_instance_id, contest=request.contest)
    problem = pi.problem
    ...
    if request.method == "POST":
        ...
        test_formset.get_time_limit_sum = types.MethodType(ValidationFormset.__dict__["get_time_limit_sum"], test_formset)
        test_formset.validate_time_limit_sum = types.MethodType(ValidationFormset.__dict__["validate_time_limit_sum"], test_formset)
        test_formset.validate_max_scores_in_group = types.MethodType(ValidationFormset.__dict__["validate_max_scores_in_group"], test_formset)
        test_formset.clean = types.MethodType(ValidationFormset.__dict__["clean"], test_formset)
...
def contest_dashboard_redirect(request):
    return redirect(reverse("simpleui_contest_dashboard", kwargs={"contest_id": request.contest.id}))
