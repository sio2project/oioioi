...
class TestContestDashboard(TestCase):
    ...
    def test_get_round_context(self):
        with fake_timezone_now(datetime(2012, 9, 8, 11, tzinfo=UTC)):
        ...
        with fake_timezone_now(datetime(2012, 6, 4, 11, tzinfo=UTC)):
            problem_instances = get_contest_problem_instances()
...
class TestProblemInstance(TestCase):
    ...
    def login(self, get_problems):
        ...
        user = User.objects.get(username="test_user")
        ContestPermission.objects.get_or_create(user=user, permission="contests.contest_admin", contest=c)
...
class TestProblemInstanceSettings(TestProblemInstance):
    ...
    def test_time_limit(self):
        ...
        self.assertContains(
            response,
            "Sum of time limits for all tests is too big. It's 51s, but it shouldn't exceed 6s.",
            html=True,
        )
...
class TestProblemInstanceValidation(TestProblemInstance):
    ...
    def test_max_scores(self):
        ...
        self.assertContains(response, "Scores for tests in the same group must be equal")
