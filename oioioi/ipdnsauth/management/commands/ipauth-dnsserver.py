            ...
            if qt in ["*", "PTR"]:
                ...
                if len(parts) == 4:
                    ...
                    try:
                        ...
                        fqdn = username_to_hostname(iptu.user.username) + "." + settings.IPAUTH_DNSSERVER_DOMAIN + "."
                        reply.add_answer(
                            RR(
                                rname=qname,
                                rtype=QTYPE.PTR,
...
class Command(BaseCommand):
    help = "DNS server for ipdnsauth.\n\nAnswers DNS queries for names and IP addresses managed by ipdnsauth module."

    def add_arguments(self, parser):
        parser.add_argument("--port", "-p", type=int, default=8053, help="Specify port to listen on")
        parser.add_argument(
            "--bind-addr",
            dest="bind_addr",
            type=str,
    ...
    def handle(self, *args, **options):
        ...
        for name, s in servers:
            thread = threading.Thread(target=s.serve_forever, name=("ipauth-dnsserver-" + name))
            thread.daemon = True
            thread.start()
