...
@override_settings(AUTHENTICATION_BACKENDS=AUTHENTICATION_BACKENDS + ("oioioi.ipdnsauth.backends.IpDnsBackend",))
@override_settings(
...
class TestAutoAuthorization(TestCase):
    ...
    def _assertBackend(self, user):
        with fake_time(datetime(2012, 1, 1, 11, tzinfo=UTC)):
            ...
            self.assertEqual(session["_auth_user_backend"], "oioioi.ipdnsauth.backends.IpDnsBackend")
...
class TestIpDnsManagement(TestCase):
    ...
    def test_ip_management(self):
        ...
        self.assertTrue(User.objects.filter(username="test_user").filter(iptouser__ip_addr="127.0.0.1").exists())
    ...
    def test_dns_management(self):
        ...
        self.assertTrue(User.objects.filter(username="test_user").filter(dnstouser__dns_name="localhost").exists())
