...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        if fields != COLUMNS:
            raise CommandError(_("Missing header or invalid columns: %(h)s. Expected: %(col)s") % {"h": ", ".join(fields), "col": ", ".join(COLUMNS)})
        ...
        with transaction.atomic():
            ...
            for row in reader:
                ...
                school, created = School.objects.get_or_create(name=row["name"], postal_code=row["postal_code"])
                if created:
                    created_count += 1
                ...
                try:
                    ...
                    for k, v in e.message_dict.items():
                        for msg in v:
                            if k == "__all__":
                                self.stdout.write(_("Line %(lineNum)s: %(msg)s\n") % {"lineNum": reader.line_num, "msg": msg})
                            else:
                                self.stdout.write(
