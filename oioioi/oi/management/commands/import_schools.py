...
BASE_DIR = rf"{os.getcwd()}/schools"
LOG_FILENAME = rf"{BASE_DIR}/school_import_log_{CURRENT_TIME}.log"
LATEST_LOG_FILENAME = rf"{BASE_DIR}/latest.log"
BACKUP_FILENAME = rf"{BASE_DIR}/rspo_{CURRENT_TIME}.back"
...
SCHOOL_FIELDS = [field.column for field in School._meta.fields if "id" not in str(field)]
...
def find_school(school: "dict[str, str] | School", schools: "list[dict[str, str]]"):
...
def are_the_same(school1: dict | School, school2: dict | School) -> bool:
...
def copy_log_file():
    ...
    with open(LOG_FILENAME, encoding="utf-8-sig") as log_file:
...
class Command(BaseCommand):
    ...
    def log(
        ...
        school_before: School | None = None,
        school_after: School | None = None,
    ):
        ...
        message_to_log = f"{message_to_log} {additional_info}" if additional_info is not None else message_to_log
    ...
    def read_rspo_csv_file(self, filename):
        ...
        with open(filename, encoding="utf-8-sig") as file:
            ...
            if reader.fieldnames is not None and not all(item in reader.fieldnames for item in RSPO_CSV_COLUMNS):
                self.log(
    ...
    def get_last_rspo_backup(self, filename=None):
        ...
        return self.read_rspo_csv_file(rf"{BASE_DIR}/{filename}")
    ...
    def handle(self, *args, **kwargs):
        ...
        if last_rspo_schools == [] and not first_import:
            raise CommandError("Make sure to set first-import if you are importing for the first time!")
        if first_import:
        ...
        if len(curr_rspo_schools) > 0:
            self.log(f"Warning: There are still {len(curr_rspo_schools)} schools left that did not have an active counterpart in the SIO database.")
        ...
        if DRY_RUN:
            ...
            self.log("IMPORTANT: The following data shows the number of queries if dry-run were disabled. Since it was on, NO CHANGES were made.")
