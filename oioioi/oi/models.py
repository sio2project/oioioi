...
class SchoolType(models.Model):
    name = models.CharField(max_length=255, validators=[validate_whitespaces], verbose_name=_("name"))
...
class School(models.Model):
    ...
    postal_code = models.CharField(
        ...
        db_index=True,
        validators=[RegexValidator(r"^\d{2}-\d{3}$", _("Enter a postal code in the format XX-XXX"))],
    )
    city = models.CharField(max_length=100, verbose_name=_("city"), db_index=True)
    province = models.CharField(max_length=100, choices=PROVINCES, verbose_name=_("province"), db_index=True)
    ...
    def is_similar(self, instance):
        ratio = difflib.SequenceMatcher(None, self.address.lower(), instance.address.lower()).ratio()
        return ratio > 0.75
...
class OIRegistration(RegistrationModel):
    ...
    t_shirt_size = models.CharField(max_length=7, choices=T_SHIRT_SIZES, verbose_name=_("t-shirt size"))
    school = models.ForeignKey(School, null=True, verbose_name=_("school"), on_delete=models.CASCADE)
    class_type = models.CharField(max_length=7, choices=CLASS_TYPES, verbose_name=_("class"))
