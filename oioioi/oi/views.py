...
from oioioi.contests.utils import can_see_personal_data, contest_exists, is_contest_admin
...
def schools_similar_view(request):
    ...
    if "address" in request.POST and request.POST["address"]:
        ...
        schools = list(schools.filter(qobj)[:num_hints]) + list(schools.filter(~qobj)[:num_hints])
