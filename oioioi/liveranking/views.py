...
def _ranking_settings(contest_id, round_id):
...
def liveranking_auto_view(request, contest_id, round_id=None):
    if not round_id:
        round_id = DEFAULT_ROUND

    ranking_variables = _ranking_settings(contest_id, round_id)
    return TemplateResponse(request, 'liveranking/auto.html',
            ranking_variables)


@enforce_condition(contest_exists & can_see_livedata)
def liveranking_simple_view(request, contest_id, round_id=None):
    if not round_id:
        round_id = DEFAULT_ROUND
...
@enforce_condition(contest_exists & can_see_livedata)
def liveranking_autoDonuts_view(request, contest_id, round_id=None):
    if not round_id:
        round_id = DEFAULT_ROUND
...
@enforce_condition(contest_exists & can_see_livedata)
def liveranking_simpleDonuts_view(request, contest_id, round_id=None):
    if not round_id:
        round_id = DEFAULT_ROUND
