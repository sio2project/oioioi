...
class SimilarityDisqualificationMixin(object):
    def is_submission_disqualified(self, submission):
        ...
        return prev or SubmissionsSimilarityEntry.objects \
            .filter(submission=submission, guilty=True).exists()

    def has_disqualification_history(self, submission):
        prev = super(SimilarityDisqualificationMixin, self) \
            .has_disqualification_history(submission)
        return prev or SubmissionsSimilarityEntry.objects \
            .filter(submission=submission).exists()

    def is_any_submission_to_problem_disqualified(self, user,
            problem_instance):
        prev = super(SimilarityDisqualificationMixin, self) \
            .is_any_submission_to_problem_disqualified(user, problem_instance)

        return prev or SubmissionsSimilarityEntry.objects \
            .filter(submission__problem_instance=problem_instance.id,
    ...
    def is_user_disqualified(self, request, user):
        prev = super(SimilarityDisqualificationMixin, self) \
            .is_user_disqualified(request, user)
