...
class Command(BaseCommand):
    help = _("Updates the list of participants of <contest_id> from the given text file (one login per line).\nLines starting with '#' are ignored.")
    ...
    def handle(self, *args, **options):
        ...
        if not issubclass(getattr(rcontroller, "participant_admin", type(None)), ParticipantAdmin):
        ...
        with transaction.atomic():
            ...
            for line in stream:
                ...
                try:
                    ...
                    _participant, created = Participant.objects.get_or_create(contest=contest, user=user)
                except User.DoesNotExist:
                    self.stdout.write(_("Error for user=%(user)s: user does not exist\n") % {"user": login})
                    ok = False
