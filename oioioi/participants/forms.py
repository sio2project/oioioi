...
class ParticipantForm(forms.ModelForm):
    ...
    def clean_user(self):
        if Participant.objects.filter(contest=self.request_contest, user=self.cleaned_data["user"]).exists() and (
            self.instance is None or self.instance.user != self.cleaned_data["user"]
        ):
            raise ValidationError(_("%s is already a participant of this contest.") % self.cleaned_data["user"].username)
...
class RegionForm(forms.ModelForm):
    ...
    def clean_short_name(self):
        if Region.objects.filter(contest=self.request_contest, short_name=self.cleaned_data["short_name"]).exists() and (
            self.instance is None or self.instance.short_name != self.cleaned_data["short_name"]
        ):
...
class TermsAcceptedPhraseForm(forms.ModelForm):
    ...
    def tag_as_str(self, tag):
        if tag in self.allowed_attributes:
            return "{} ({})".format(tag, ", ".join(sorted(self.allowed_attributes[tag])))
        else:
            return tag
    ...
    def __init__(self, *args, **kwargs):
        ...
        if "text" in self.fields:
            ...
            self.fields["text"].help_text = _("You can use the following tags and attributes: {}.").format(
