...
class TestParticipantsContestViews(TestCase):
    ...
    def test_participants_contest_visibility(self):
        ...
        contest.controller_name = "oioioi.participants.tests.ParticipantsContestController"
        contest.save()
...
class TestParticipantsRegistration(TestCase):
    ...
    def test_participants_registration(self):
        ...
        register_url = reverse("participants_register", kwargs={"contest_id": contest.id})
...
class TestOnsiteAdmin(TestCase):
    ...
    def setUp(self):
        ...
        self.contest.controller_name = "oioioi.participants.tests.OnsiteContestController"
        self.contest.save()
    ...
    def test_participants_import(self):
        ...
        filename = os.path.join(os.path.dirname(__file__), "files", "onsite_participants.csv")
        manager = import_onsite_participants.Command()
        manager.run_from_argv(["manage.py", "import_onsite_participants", str(self.contest.id), filename])
...
class TestParticipantsModelAdmin(TestCase):
    ...
    def test_noadmin_admin_visibility(self):
        contest = Contest.objects.get()
        contest.controller_name = "oioioi.participants.tests.NoAdminParticipantsContestController"
        contest.save()
        user = User.objects.get(username="test_user")
    ...
    def test_participants_import(self):
        ...
        manager.run_from_argv(["manage.py", "import_participants", str(contest.id), filename])
...
@override_settings(
    MIDDLEWARE=MIDDLEWARE + ("oioioi.contestexcl.middleware.ExclusiveContestsMiddleware",),
    ROOT_URLCONF="oioioi.contests.tests.test_urls",
)
class TestParticipantsExclusiveContestsMiddlewareMixin(TestCase, ContestIdViewCheckMixin):
    ...
    def test_participants_selector(self):
        self.c1.controller_name = "oioioi.participants.tests.ParticipantsContestController"
        self.c1.save()
    ...
    def test_contest_admin_with_participant(self):
        self.c2.controller_name = "oioioi.participants.tests.ParticipantsContestController"
        self.c2.save()

        ContestPermission(user=self.user, contest=self.c1, permission="contests.contest_admin").save()
...
class TestRegistrationModel(TestCase):
    ...
    def test_both_hands_cascading_on_registration_delete(self):
        def _assert_equals_len(expectedLen=None):
            self.assertEqual(Participant.objects.count(), TestRegistration.objects.count())
            if expectedLen:
...
class TestAnonymousParticipants(TestCase):
    ...
    def test_no_anonymous_participants(self):
        ...
        with fake_timezone_now(datetime(2015, 8, 5, tzinfo=UTC)):
            ...
            self.assertFalse(re.search(user_pattern % ("test_user",), response.content.decode("utf-8")))
            self.assertTrue(re.search(user_pattern % ("Test User",), response.content.decode("utf-8")))
    ...
    def test_anonymous_participants(self):
        ...
        with fake_timezone_now(datetime(2015, 8, 5, tzinfo=UTC)):
            ...
            self.assertFalse(re.search(user_pattern % ("test_user",), response.content.decode("utf-8")))
            self.assertTrue(re.search(user_pattern % ("Test User",), response.content.decode("utf-8")))
            self.assertTrue(re.search(user_pattern % ("test_user2",), response.content.decode("utf-8")))
            self.assertFalse(re.search(user_pattern % ("Test User 2",), response.content.decode("utf-8")))
            ...
            self.assertTrue(self.client.login(username="test_contest_admin"))
            response = self.client.get(url)
            self.assertFalse(re.search(user_pattern % ("test_user2",), response.content.decode("utf-8")))

            self.assertTrue(re.search(user_pattern % ("Test User 2",), response.content.decode("utf-8")))
...
class TestParticipantsDataViews(TestCase):
    ...
    def test_no_email_data_view(self):
        ...
        perm = ContestPermission(user=user, contest=contest, permission="contests.personal_data")
        perm.save()
    ...
    def test_data_view(self):
        ...
        try:
            ...
            response = self.client.get(url)
            self.assertContains(response, f"<td>{user.id}</td>")
    ...
    def test_none_school(self):
        ...
        perm = ContestPermission(user=user2, contest=contest, permission="contests.personal_data")
        perm.save()
...
class TestOnsiteRegistration(TestCase):
    ...
    def test_missing_registration_model(self):
        ...
        self.assertRaises(ObjectDoesNotExist, lambda: p.registration_model)
