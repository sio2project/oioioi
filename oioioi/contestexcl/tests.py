...
class TestExclusiveContestsAdmin(TestCase, ContestIdViewCheckMixin):
    ...
    def _modify_contestexcl(
        ...
        formsets = (
            ...
            ("oi_data_confirmation_settings", 0, 0, 0, 1),
        ...
        for name, total, initial, min_num, max_num in formsets:
            data.update(
                {
                    f"{name}-TOTAL_FORMS": total,
                    f"{name}-INITIAL_FORMS": initial,
                    f"{name}-MIN_NUM_FORMS": min_num,
                    f"{name}-MAX_NUM_FORMS": max_num,
                }
            )
        data.update(
            {
        ...
        for i in range(len(excl_start_date_forms)):
            data.update(
                {
                    f"exclusivenessconfig_set-{i}-id": "",
                    f"exclusivenessconfig_set-{i}-contest": "c",
                    f"exclusivenessconfig_set-{i}-enabled": "on",
                    f"exclusivenessconfig_set-{i}-start_date_0": excl_start_date_forms[i][0],
                    f"exclusivenessconfig_set-{i}-start_date_1": excl_start_date_forms[i][1],
                    f"exclusivenessconfig_set-{i}-end_date_0": "",
                    f"exclusivenessconfig_set-{i}-end_date_1": "",
                }
            )
        for i in range(len(excl_end_date_forms)):
            data.update(
                {
                    f"exclusivenessconfig_set-{i}-end_date_0": excl_end_date_forms[i][0],
                    f"exclusivenessconfig_set-{i}-end_date_1": excl_end_date_forms[i][1],
                }
            )

        post_url = reverse("oioioiadmin:contests_contest_change", args=[self.c.id]) + "?simple=true"
        response = self.admin.post(post_url, data, follow=True)
        self.assertEqual(response.status_code, 200)
        return response

    def test_exclusiveness_round_warning(self):
        response = self._modify_contestexcl(("2019-01-01", "10:00:00"), ("", ""), [("2019-01-01", "12:00:00")], [])
        self.assertContains(response, "is not exclusive from")

        response = self._modify_contestexcl(("2019-01-01", "10:00:00"), ("", ""), [("2019-01-01", "10:00:00")], [])
        ...
        for h in (7, 9, 11):
            response = self._modify_contestexcl(
                ("2019-01-01", "10:00:00"),
                ("", ""),
                [("2019-01-01", f"{h}:00:00")],
                [("2019-01-01", f"{h + 2}:00:00")],
            )
            self.assertContains(response, "is not exclusive from")
...
class TestExclusiveContests(TestCase, ContestIdViewCheckMixin):
    ...
    def test_exclusive_contests_error(self):
        ...
        with fake_time(datetime(2012, 1, 1, 13, tzinfo=UTC)):
            ...
            self.assertContains(response, "participate in more than one contest that exc")
