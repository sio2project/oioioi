...
def submit_view(request):
    try:
        ...
        submission = request.contest.controller.create_submission(request, pi, form.cleaned_data)
        result_url = reverse(
            "submission",
        ...
        result = {
            ...
            "error_data": exception_info.args[1] if len(exception_info.args) == 2 else "",
        }
...
def view_user_token(request):
    ...
    return TemplateResponse(
        ...
        "submitservice/view-user-token.html",
        {
            ...
            "contest_url": request.build_absolute_uri(reverse("default_contest_view", kwargs={"contest_id": request.contest.id})),
        },
    )
...
def clear_user_token(request):
    ...
    return redirect(reverse("submitservice_view_user_token", kwargs={"contest_id": request.contest.id}))
