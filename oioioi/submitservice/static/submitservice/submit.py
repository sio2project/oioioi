...
def main():
    ...
    save_config_and_configure_group.add_argument(
        ...
        "--save-config",
        action="store_true",
        help="used along with --token or/and --url, saves " + "configuration changes to configuration file",
    )
    ...
    parser.add_argument(
        ...
        "--task",
        action="store",
        help="override task short name (if not specified, " + "filename with extension is taken)",
    )
    parser.add_argument("-k", "--token", action="store", help="provide a token for authentication")
    parser.add_argument(
        "-u",
        "--url",
        action="store",
...
def submit(filename, task_name, token, contest_url, open_webbrowser):
    if "contest-url" not in configuration:
        print(
            "Configuration file could not be loaded. " + "Please run submit.py --configure " + "to configure the application.",
            file=sys.stderr,
        )
        return 1
    ...
    try:
        ...
        if "error_code" not in result:
            ...
            raise RuntimeError(error_code_map[result["error_code"]] % {"data": result["error_data"]})
...
def query(key, value_friendly_name, mask_old_value=False):
    ...
    if not old_value:
        old_value = ""
    new_value = input("[%s] " % (key if mask_old_value and key in configuration else old_value))
    if new_value:
...
def create_configuration():
    ...
    while query("contest-url", "contest website (e.g. http://oioioi.com/c/example-contest/)"):
        pass
