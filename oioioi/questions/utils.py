...
from oioioi.questions.models import AddQuestionMessage, NewsMessage
...
def unanswered_questions(messages):
    return messages.filter(message__isnull=True, top_reference__isnull=True, marked_read_by__isnull=True, kind="QUESTION")


def get_news_message(request):
    return get_public_message(request, NewsMessage, "questions_news_message")
