...
def notification_function_answer(arguments):
    assert hasattr(arguments, "user") and hasattr(arguments, "question_instance") and hasattr(arguments, "answer_instance"), (
        "The log doesn't have user, question_instance or answer_instance value in the extra map"
    )

    message_details = arguments.question_instance.topic + ": " + arguments.question_instance.content
    ...
    NotificationHandler.send_notification(arguments.user, "question_answered", message, message_arguments)
...
def notification_function_public(arguments):
    ...
    for user in controller.users_to_receive_public_message_notification():
        NotificationHandler.send_notification(user, "new_public_message", message, message_arguments)


NotificationHandler.register_notification("new_public_message", notification_function_public)
