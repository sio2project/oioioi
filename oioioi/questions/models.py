...
class Message(models.Model):
    ...
    date = models.DateTimeField(default=timezone.now, editable=False, verbose_name=_("date"))
    ...
    mail_sent = models.BooleanField(default=False, verbose_name=_("mail notification sent"))
    ...
    def save(self, *args, **kwargs):
        ...
        if self.top_reference:
            related = Message.objects.filter(Q(id=self.top_reference_id) | Q(top_reference_id=self.top_reference_id))
        elif self.id:
    ...
    def get_absolute_url(self):
        link = reverse(
            ...
            kwargs={
                ...
                "message_id": self.top_reference_id if self.top_reference_id is not None else self.id,
            },
        )
        return link
...
def send_notification(sender, instance, created, **kwargs):
    ...
    if instance.kind == "PUBLIC" and instance.contest is not None:
        if instance.problem_instance is not None:
            logger.info(
                'Public message "%(topic)s" about problem "%(short_name)s" was created',
                {
