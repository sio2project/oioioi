...
from oioioi.publicsolutions.utils import get_public_solutions, \
    get_may_be_published_solutions, solution_may_be_published, \
    any_round_public
...
@menu_registry.register_decorator(_("Solutions"), lambda request:
    reverse('list_solutions',
        kwargs={'contest_id': request.contest.id}), order=1000)
@enforce_condition(contest_exists & can_enter_contest)
@enforce_condition(any_round_public)
def list_solutions_view(request, contest_id):
    ...
    subs = subs.order_by('problem_instance', 'user__last_name')

    return TemplateResponse(request,
            'publicsolutions/list_solutions.html', {
                'submissions': subs, 'form': form,
                'submissions_on_page':
...
@enforce_condition(any_round_public)
def publish_solutions_view(request, contest_id):
    subs = get_may_be_published_solutions(request) \
...
@require_POST
def publish_solution_view(request, contest_id, submission_id):
