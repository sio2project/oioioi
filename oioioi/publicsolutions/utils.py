...
def public_rounds(request):
    ...
    return [round for round in Round.objects.filter(contest=request.contest)
                  if controller.can_see_publicsolutions(request, round)]


@request_cached
def public_problem_instances(request):
    return ProblemInstance.objects.filter(round__in=public_rounds(request))
...
def unfiltered_submissions(request):
    return Submission.objects \
            ...
            .select_related('publication', 'user', 'problem_instance')
...
def get_public_solutions(request):
    ...
    return controller.solutions_must_be_public(subs) | \
           subs.filter(publication__isnull=False)
...
def get_may_be_published_solutions(request):
    ...
    return controller.solutions_may_be_published(
            subs.exclude(id__in=mustbe_subs))


@request_cached
def problem_instances_with_any_public_solutions(request):
    return public_problem_instances(request) \
...
def any_round_public(request):
    return \
        isinstance(request.contest.controller, ProgrammingContestController) \
