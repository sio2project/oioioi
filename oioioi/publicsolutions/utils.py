...
def public_problem_instances(request):
    return filter_public_problem_instances(request,
                                           ProblemInstance.objects.all())
...
def filter_public_solutions(request, qs):
    ...
    return (cc.solutions_must_be_public(qs) |
            qs.filter(publication__isnull=False)).filter(
