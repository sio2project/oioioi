...
def _is_suspended(problem_instance_id, suspend_init_tests=None):
    if suspend_init_tests is None:
        return SuspendedProblem.objects.filter(problem_instance=problem_instance_id).exists()
    else:
        return SuspendedProblem.objects.filter(problem_instance=problem_instance_id, suspend_init_tests=suspend_init_tests).exists()
...
def _is_admin_submission(env, submission):
    if submission.user is not None:
        return submission.user.has_perm("contests.contest_admin", submission.problem_instance.contest)
    return False
